...
function buildMobileUi(): MobileUi {
  ...
  return {
    root, rotate, movePad, moveThumb, lookPad, lookThumb, interact, fire, fullscreen, actionRail, actionUp, actionSelect, actionDown
  };
}
...
class MobileControlsImpl implements MobileControls {
  ...
  private pulseTimers = new Map<BooleanInputKey, ReturnType<typeof setTimeout>>();

  constructor(input: InputState, options: MobileControlsOptions) {
    this.input = input;
    this.options = options;
    this.ui = buildMobileUi();
    document.body.append(this.ui.root);

    this.bindPad(this.ui.movePad, this.ui.moveThumb, 'move');
    this.bindPad(this.ui.lookPad, this.ui.lookThumb, 'look');
    this.holdInteractButton(this.ui.interact);

    this.ui.fullscreen.addEventListener('pointerdown', e => {
    ...
    this.ui.fire.addEventListener('pointerdown', e => {
      if (!this.enabled || !this.context.started || this.context.menuOpen || this.context.gameOver) return;
      e.preventDefault();
      e.stopPropagation();
      this.options.onGesture();
      this.firePointer = e.pointerId;
      capturePointer(this.ui.fire, e.pointerId);
      this.input.mouseAttack = true;
    });
    const endFire = (e: PointerEvent): void => {
      if (e.pointerId !== this.firePointer) return;
      e.preventDefault();
      releasePointer(this.ui.fire, e.pointerId);
      this.firePointer = -1;
      this.input.mouseAttack = false;
    };
    this.ui.fire.addEventListener('pointerup', endFire);
    this.ui.fire.addEventListener('pointercancel', endFire);
    this.ui.fire.addEventListener('lostpointercapture', endFire);

    this.ui.actionUp.addEventListener('pointerdown', e => {
      if (!this.enabled || !this.context.started) return;
      e.preventDefault();
      e.stopPropagation();
      this.actionNav(-1);
    });
    this.ui.actionDown.addEventListener('pointerdown', e => {
      if (!this.enabled || !this.context.started) return;
      e.preventDefault();
      e.stopPropagation();
      this.actionNav(1);
    });
    this.ui.actionSelect.addEventListener('pointerdown', e => {
      if (!this.enabled || !this.context.started) return;
      e.preventDefault();
      e.stopPropagation();
    ...
    const actionEnd = (e: PointerEvent): void => {
      ...
      e.preventDefault();
      releasePointer(this.ui.actionSelect, e.pointerId);
      this.releaseHeldAction();
    };
    this.ui.actionSelect.addEventListener('pointerup', actionEnd);
    this.ui.actionSelect.addEventListener('pointercancel', actionEnd);
    this.ui.actionSelect.addEventListener('lostpointercapture', actionEnd);
  ...
  private syncHudSafeContext(): void {
    ...
    this.ui.root.style.setProperty('--mobile-safe-top-px', `${cssInsets.top}px`);
    this.ui.root.style.setProperty('--mobile-safe-right-px', `${cssInsets.right}px`);
    this.ui.root.style.setProperty('--mobile-safe-bottom-px', `${cssInsets.bottom}px`);
    this.ui.root.style.setProperty('--mobile-safe-left-px', `${cssInsets.left}px`);
  }
  ...
  private clearTouchInput = (): void => {
    ...
    this.lookPointer = -1;
    this.firePointer = -1;
    this.ui.moveThumb.style.transform = '';
    this.ui.lookThumb.style.transform = '';
  }

  private updateLocalizedText(): void {
    this.ui.rotate.textContent = mobileText({ ru: 'Поверните телефон горизонтально', en: 'Rotate phone landscape' });
    this.ui.movePad.setAttribute('aria-label', mobileText({ ru: 'Джойстик движения', en: 'Movement stick' }));
    this.ui.lookPad.setAttribute('aria-label', mobileText({ ru: 'Джойстик камеры', en: 'Camera stick' }));
    this.ui.interact.textContent = mobileInteractLabel();
    this.ui.interact.setAttribute('aria-label', mobileText({ ru: 'Взаимодействие', en: 'Interact' }));
    this.ui.fire.setAttribute('aria-label', mobileText({ ru: 'Атака', en: 'Attack' }));
    this.ui.actionUp.setAttribute('aria-label', mobileText(this.context.menuOpen ? { ru: 'Меню выше', en: 'Menu up' } : { ru: 'Действие выше', en: 'Previous action' }));
    this.ui.actionDown.setAttribute('aria-label', mobileText(this.context.menuOpen ? { ru: 'Меню ниже', en: 'Menu down' } : { ru: 'Действие ниже', en: 'Next action' }));
  }

  private updateSelectedLabel(): void {
    const action = MOBILE_ACTIONS[this.selectedAction];
    this.ui.actionSelect.textContent = this.context.menuOpen
      ? (this.context.gameOver ? 'R' : mobileText({ ru: 'ЗАКР', en: 'CLOSE' }))
      : mobileText(action.label);
    this.ui.actionSelect.setAttribute(
      'aria-label',
  ...
  private updateFullscreenUi(): void {
    ...
    this.ui.fullscreen.dataset.fullscreenMode = mode;
    this.ui.fullscreen.hidden = standalone || (!embedded && !nativeFullscreen);
    this.ui.fullscreen.textContent = embedded ? 'PAGE' : (active ? 'EXIT' : 'FULL');
    this.ui.fullscreen.setAttribute(
      'aria-label',
      embedded
  ...
  private setEnabled(next: boolean): void {
    this.enabled = next;
    this.ui.root.toggleAttribute('hidden', !this.enabled);
  ...
  private refreshClasses(): void {
    ...
    this.ui.root.classList.toggle('is-started', this.context.started);
    this.ui.root.classList.toggle('is-menu-open', this.context.menuOpen);
    this.ui.root.classList.toggle('can-interact', this.context.canInteract);
    this.ui.root.classList.toggle('is-game-over', this.context.gameOver);
    this.ui.root.classList.toggle('is-portrait', portrait);
  ...
  private actionConfirm(e: PointerEvent): void {
    ...
    if (this.context.gameOver) {
      ...
      if (action.kind === 'menu') {
        ...
        capturePointer(this.ui.actionSelect, e.pointerId);
