...
class MobileControlsImpl implements MobileControls {
  ...
  constructor(private readonly input: InputState, private readonly options: MobileControlsOptions) {
  ...
  private bindFullscreen(): void {
    this.fullscreen.addEventListener('pointerdown', (e: PointerEvent) => {
  ...
  private bindFire(): void {
    this.fire.addEventListener('pointerdown', (e: PointerEvent) => {
      if (!this.enabled || !this.context.started || this.context.menuOpen || this.context.gameOver) return;
      e.preventDefault();
      e.stopPropagation();
      this.options.onGesture();
  ...
  private bindActionRail(): void {
    this.actionUp.addEventListener('pointerdown', (e: PointerEvent) => {
      if (!this.enabled || !this.context.started) return;
      e.preventDefault();
      e.stopPropagation();
      this.actionNav(-1);
    });
    this.actionDown.addEventListener('pointerdown', (e: PointerEvent) => {
      if (!this.enabled || !this.context.started) return;
      e.preventDefault();
      e.stopPropagation();
      this.actionNav(1);
    });
    this.actionSelect.addEventListener('pointerdown', (e: PointerEvent) => {
      if (!this.enabled || !this.context.started) return;
      e.preventDefault();
      e.stopPropagation();
