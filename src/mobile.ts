...
class MobileControlsImpl implements MobileControls {
  ...
  private movePad!: HTMLButtonElement;
  private moveThumb!: HTMLSpanElement;
  private lookPad!: HTMLButtonElement;
  private lookThumb!: HTMLSpanElement;
  ...
  private actionUp!: HTMLButtonElement;
  private actionSelect!: HTMLButtonElement;
  private actionDown!: HTMLButtonElement;
  ...
  private onResizeHandler = (): void => this.refresh();

  constructor(private input: InputState, private options: MobileControlsOptions) {
  ...
  private createDomElements() {
    ...
    this.movePad = makeButton('mobile-pad mobile-pad--move', '', 'Джойстик движения');
    this.moveThumb = document.createElement('span');
    this.moveThumb.className = 'mobile-pad-thumb';
    this.movePad.append(this.moveThumb);

    this.lookPad = makeButton('mobile-pad mobile-pad--look', '', 'Джойстик камеры');
    this.lookThumb = document.createElement('span');
    this.lookThumb.className = 'mobile-pad-thumb';
    this.lookPad.append(this.lookThumb);

    this.interact = makeButton('mobile-interact', mobileInteractLabel(), mobileText({ ru: 'Взаимодействие', en: 'Interact' }));
    this.fire = makeButton('mobile-fire-zone', '', mobileText({ ru: 'Атака', en: 'Attack' }));
    this.fullscreen = makeButton('mobile-fullscreen', 'FULL', mobileText({ ru: 'Полный экран', en: 'Fullscreen' }));

    this.actionRail = document.createElement('div');
    this.actionRail.className = 'mobile-menu-rail';
    this.actionUp = makeButton('mobile-menu-btn', '▲', mobileText({ ru: 'Действие выше', en: 'Previous action' }));
    this.actionSelect = makeButton('mobile-menu-btn mobile-menu-select', mobileText(MOBILE_ACTIONS[0].label), mobileText(MOBILE_ACTIONS[0].ariaLabel));
    this.actionDown = makeButton('mobile-menu-btn', '▼', mobileText({ ru: 'Действие ниже', en: 'Next action' }));
    this.actionRail.append(this.actionUp, this.actionSelect, this.actionDown);

    this.root.append(this.rotate, this.fire, this.fullscreen, this.movePad, this.lookPad, this.interact, this.actionRail);
  ...
  private bindInteract(): void {
    ...
    this.interact.addEventListener('pointerdown', e => {
      ...
      capturePointer(this.interact, e.pointerId);
    ...
    const pointerEnd = (e: PointerEvent): void => {
      ...
      e.preventDefault();
      releasePointer(this.interact, e.pointerId);
      interactPointer = -1;
      this.input.interact = false;
      this.input.interactHeld = false;
    };
    this.interact.addEventListener('pointerup', pointerEnd);
    this.interact.addEventListener('pointercancel', pointerEnd);
    this.interact.addEventListener('lostpointercapture', pointerEnd);
  }
  ...
  private bindWindowEvents(): void {
    ...
    window.visualViewport?.addEventListener('scroll', this.onResizeHandler);
    document.addEventListener('fullscreenchange', this.onResizeHandler);
    document.addEventListener('webkitfullscreenchange', this.onResizeHandler);
  }
  ...
  destroy(): void {
    ...
    window.visualViewport?.removeEventListener('scroll', this.onResizeHandler);
    document.removeEventListener('fullscreenchange', this.onResizeHandler);
    document.removeEventListener('webkitfullscreenchange', this.onResizeHandler);
