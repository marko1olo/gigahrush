...
function populateKvartiry(world: World, entities: Entity[], nextId: number): number {
...
function buildKvartiryInfrastructure(world: World, territorySeed: number): void {
  const DX = [1, 0, -1, 0];
  const DY = [0, 1, 0, -1];
...
function buildKvartiryRooms(world: World): void {
  const DX = [1, 0, -1, 0];
  const DY = [0, 1, 0, -1];
  let nextRoomId = 0;
...
function buildKvartiryWallsAndDoors(world: World): void {
  const DX = [1, 0, -1, 0];
  const DY = [0, 1, 0, -1];
...
export function generateKvartiry(territorySeed = 0): { world: World; entities: Entity[]; spawnX: number; spawnY: number } {
  ...
  buildKvartiryWallsAndDoors(world);

  buildKvartiryRooms(world);

  buildKvartiryInfrastructure(world, territorySeed);

  nextId = populateKvartiry(world, entities, nextId);
