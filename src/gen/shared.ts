...
export function findClearArea(
  ...
  for (let attempt = 0; attempt < 400; attempt++) {
    const angle = (rng(0, 1000) / 1000) * Math.PI * 2;
    const lo = attempt < 200 ? minDist : 0;
    const hi = attempt < 200 ? maxDist : W / 4;
    const dist = lo + (rng(0, 1000) / 1000) * (hi - lo);
  ...
  for (let y = 0; y < W; y++) {
    for (let x = 0; x < W; x++) {
      ...
      for (const [dx, dy] of [[1, 0], [0, 1]] as const) {
        ...
        if (!crossesZone && (rng(0, 1000) / 1000) > 0.01) continue;
