...
const LIQUIDATOR_DEF: PlotNpcDef = {
  ...
  talkLines: [
    'Труп после самосбора не трогают руками. Сначала смотрят, дышит ли пыль.',
    'Мухи обходят койку - значит, там кто-то считает добычу вместо нас.',
...
function suspiciousCorpseInventory(room: Room): WorldContainer['inventory'] {
  const inventory: WorldContainer['inventory'] = [
    ...
    { defId: 'rawmeat', count: 1 },
    {
      defId: 'note',
      count: 1,
      data: 'Лут слишком чистый. Тело дышит пылью, мухи держатся у стены. Если берешь - бей первым или жги.',
    },
  ];
...
function seedScene(world: World, room: Room, entities: Entity[], nextId: { v: number }, liquidatorId: number): void {
  addDrop(
    ...
    'note',
    1,
    'Записка обхода: тело за гермодверью дышит пылью. Мух нет. Лут слишком чистый. Не лезь без огня или дистанции.',
  );
