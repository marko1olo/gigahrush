...
const NPC_DEFS: Record<string, PlotNpcDef> = {
  [MARKER_ID]: {
    ...
    talkLinesPost: [
      ...
      'Если метку сорвали, считай, дверь снова представилась соседкой.',
    ],
  },
  [LIQUIDATOR_ID]: {
    name: 'Юрий Пломба',
    ...
    talkLinesPost: [
      'Пломба не делает дверь доброй. Она делает решение дешевле.',
  ...
  [HUNTER_ID]: {
    ...
    talkLinesPost: [
      'Голос стал предметом. Это лучший исход для голоса.',
      'Когда дверь молчит после зачистки, не благодарите ее. Она запоминает вежливых.',
    ],
  },
};
...
registerSideQuest(LIQUIDATOR_ID, NPC_DEFS[LIQUIDATOR_ID], [{
  ...
  type: QuestType.FETCH,
  desc: 'Юрий Пломба: «Один тюбик герметика - и я дожму петлю. Открывать не надо, если можно заставить дверь молчать.»',
...
function spawnGolosMonster(world: World, entities: Entity[], nextId: { v: number }, x: number, y: number): void {
  ...
  applyMonsterVariant(monster, FloorLevel.LIVING, false);
