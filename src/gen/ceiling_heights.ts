...
export function stampCeilingHeights(world: World): void {
  ...
  for (let y = 0; y < W; y++) {
    ...
    for (let x = 0; x < W; x++) {
      ...
      // When a global ceiling is active (like the sky), prevent walls from rising to the sky level.
      // This assigns a deterministic pseudo-random height to create a varied skyline of parapets.
      if (world.globalCeilingTier !== undefined && m === world.globalCeilingTier) {
        const rid = roomMap[i];
        if (rid >= 0) {
          // Walls belonging to a room receive a consistent height.
          ...
          // Abyss walls with no room use a chunked spatial hash.
