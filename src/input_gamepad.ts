...
function pollSticks(pad: Gamepad, settings: GamepadSettings, frame: InputFrame): boolean {
  let anyInput = false;
  const axes = pad.axes;
  if (axes.length >= 2) {
...
function pollButtons(pad: Gamepad, state: GamepadAdapterState, settings: GamepadSettings, frame: InputFrame): boolean {
  let anyInput = false;
...
export function createGamepadAdapter(): GamepadAdapter {
  ...
  function poll(frame: InputFrame): void {
    ...
    // ── Sticks ──────────────────────────────────────────────
    if (pollSticks(pad, settings, frame)) {
      anyInput = true;
    }

    // ── Buttons & edges ─────────────────────────────────────
    if (pollButtons(pad, state, settings, frame)) {
      anyInput = true;
    }
