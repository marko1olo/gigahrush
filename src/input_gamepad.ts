...
function pollButtons(pad: Gamepad, frame: InputFrame, settings: GamepadSettings, state: GamepadAdapterState): boolean {
  let anyInput = false;
...
export function createGamepadAdapter(): GamepadAdapter {
  ...
  function poll(frame: InputFrame): void {
    ...
    const sticksInput = pollSticks(pad, frame, settings);
    const buttonsInput = pollButtons(pad, frame, settings, state);

    if (sticksInput || buttonsInput) {
