...
export function generateSprite(): Uint32Array {
  ...
  for (let i = 0; i < 9; i++) {
    ...
    line(t, cx + side * 2, 53, cx + side * len, y0 + 4, rgba(42, 74, 36, 230));
  }
  ...
  for (let y = 12; y < 56; y++) {
    ...
    for (let x = Math.floor(cx + sway - half); x <= Math.ceil(cx + sway + half); x++) {
      ...
      put(t, x, y, rgba(clamp(112 + n - shade), clamp(162 + n - shade), clamp(90 + n * 0.45 - shade)));
    }
    if (y % 7 === 0) line(t, cx + sway - 3, y, cx + sway + 3, y + 1, rgba(62, 92, 52, 225));
  }

  for (let i = 0; i < 8; i++) {
    const y = 22 + Math.floor(noise(i, 4, 619) * 25);
    const x = Math.floor(cx + Math.sin(y * 0.18) * 2 + (noise(i, 5, 620) - 0.5) * 8);
    ellipse(t, x, y, 2.2, 3.2, rgba(58, 22, 58, 235), 621 + i);
    put(t, x + 1, y, rgba(220, 198, 52, 245));
  }
  ...
  for (let i = 0; i < 7; i++) {
    ...
    line(t, cx + side * 2, y, lx, ly, rgba(58, 120, 54, 240));
    ellipse(t, lx, ly, 7.5, 3.8, rgba(60, 132, 62, 230), 640 + i);
    for (let j = -2; j <= 2; j++) line(t, lx, ly, lx + side * (4 + Math.abs(j)), ly + j * 2, rgba(44, 96, 42, 205));
  }
  ...
  for (let i = 0; i < 18; i++) {
    ...
    line(t, cx, 16, ex, ey, rgba(112, 158, 96, 180));
    ellipse(t, ex, ey, 1.7, 1.7, rgba(232, 232, 216, 245), 660 + i);
    if (i % 3 === 0) put(t, Math.round(ex + 1), Math.round(ey), rgba(244, 238, 176, 245));
  }
  ellipse(t, cx, 16, 3.2, 3.2, rgba(224, 230, 204, 255), 699);
