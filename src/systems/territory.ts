...
export function territoryOwnerAtIndex(
  world: World,
  idx: number,
): TerritoryOwner {
...
export function setTerritoryOwnerAtIndex(
  world: World,
  idx: number,
  owner: TerritoryOwner,
): boolean {
...
export function setTerritoryOwnerAt(
  world: World,
  x: number,
  y: number,
  owner: TerritoryOwner,
): boolean {
  return setTerritoryOwnerAtIndex(
    world,
    world.idx(Math.floor(x), Math.floor(y)),
    owner,
  );
}
...
export function dominantTerritoryOwnerInRoom(
  world: World,
  roomId: number,
): TerritoryOwner {
...
export function territoryRoomOwner(
  world: World,
  roomId: number,
): TerritoryOwner {
  const room = world.rooms[roomId];
...
export function currentTerritoryZoneId(
  world: World,
  x: number,
  y: number,
): number {
...
function seedTerritoryFromZoneMetadata(world: World): void {
  for (let i = 0; i < W * W; i++) {
    ...
    world.factionControl[i] = normalizeOwner(
      zone?.faction ?? ZoneFaction.CITIZEN,
    );
  }
}

function dominantOwnerFromCounts<T extends Uint16Array | Uint32Array>(
  counts: T,
  fallback: TerritoryOwner,
): TerritoryOwner {
  let best = fallback;
  let bestCount = -1;
...
function roomCenter(room: Room): { x: number; y: number } {
  return {
    x: worldWrap(room.x + (room.w >> 1)),
    y: worldWrap(room.y + (room.h >> 1)),
  };
}
...
function autoHqRoomSpanEligible(room: Room): boolean {
  return (
    room.w > 1 &&
    room.h > 1 &&
    room.w <= AUTO_HQ_MAX_ROOM_SPAN &&
    room.h <= AUTO_HQ_MAX_ROOM_SPAN
  );
}
...
function forEachMappedRoomCell(
  world: World,
  room: Room,
...
function roomHasHermeticDoor(world: World, room: Room): boolean {
  return room.doors.some((idx) => {
    ...
    return (
      state === DoorState.HERMETIC_OPEN || state === DoorState.HERMETIC_CLOSED
    );
  });
}
...
function roomPreference(owner: TerritoryOwner, room: Room): number {
  ...
  if (owner === ZoneFaction.LIQUIDATOR) {
    if (room.type === RoomType.OFFICE || room.type === RoomType.STORAGE)
      return 40;
    if (room.type === RoomType.CORRIDOR || room.type === RoomType.COMMON)
      return 20;
  }
  if (owner === ZoneFaction.CULTIST) {
    if (room.type === RoomType.COMMON || room.type === RoomType.STORAGE)
      return 36;
...
function hqAnchorSelectionScore(
  world: World,
  room: Room,
  owner: TerritoryOwner,
): number {
...
function pushTerritoryHqAnchor(
  anchors: TerritoryHqAnchor[],
  room: Room,
  owner: TerritoryOwner,
): void {
...
export function paintTerritoryDisc(
  ...
  const cap = Math.max(
    0,
    Math.floor(options.cellCap ?? Number.MAX_SAFE_INTEGER),
  );
  ...
  for (let dy = -r; dy <= r; dy++) {
    for (let dx = -r; dx <= r; dx++) {
      ...
      if (options.zoneId !== undefined && world.zoneMap[idx] !== options.zoneId)
        continue;
      if (options.passableOnly && !walkableForCapture(world.cells[idx]))
        continue;
...
function paintOwnerPatch(
  ...
  for (let dy = -HQ_PATCH_RADIUS; dy <= HQ_PATCH_RADIUS; dy++) {
    for (let dx = -HQ_PATCH_RADIUS; dx <= HQ_PATCH_RADIUS; dx++) {
      ...
      if (world.cells[idx] === Cell.ABYSS || world.cells[idx] === Cell.LIFT)
        continue;
      if (setTerritoryOwnerAtIndex(world, idx, owner)) changed++;
    }
  }
...
function hardenHqRoom(world: World, room: Room, owner: TerritoryOwner): void {
  ...
  if (
    (existingHq && !hqRoomGeometrySane(room)) ||
    (!existingHq && !autoHqCandidateEligible(world, room))
  ) {
    room.sealed = false;
  ...
  if (
    !room.name ||
    room.name.startsWith("Комната") ||
    room.name.startsWith("Миништаб")
  ) {
...
function reinforceHqSupportRooms(
  world: World,
  hq: Room,
  owner: TerritoryOwner,
): void {
...
export function territoryHqAnchors(world: World): TerritoryHqAnchor[] {
  ...
  for (const owner of HUMAN_TERRITORY_OWNERS) {
    const room = chooseExistingHqAnchorRoom(
      world,
      owner,
      usedRooms,
      hqAnchorEligible,
...
function ensureMiniHqPatches(world: World): void {
  ...
  for (const owner of HUMAN_TERRITORY_OWNERS) {
    ...
    for (const anchor of territoryHqAnchors(world)) {
      if (
        anchor.owner === owner &&
        world.rooms[anchor.roomId]?.type === RoomType.HQ
      ) {
        hasAnchor = true;
        break;
      }
...
function territoryHash01(seed: number, a: number, b: number, c = 0): number {
  let h =
    seed ^
    Math.imul(a + 0x9e37, 0x85ebca6b) ^
    Math.imul(b + 0x632b, 0xc2b2ae35) ^
    Math.imul(c + 0x27d4, 0x165667b1);
  h ^= h >>> 16;
...
function normalizedTargetShares(
  shares: readonly TerritoryTargetShare[] | undefined,
): TerritoryTargetShare[] {
  if (!shares || shares.length === 0) return [];
  const rows = shares
    .filter(
      (row) =>
        TERRITORY_OWNERS.includes(row.owner) &&
...
function ownerBucketScore(
  world: World,
  owner: TerritoryOwner,
  anchors: readonly TerritoryHqAnchor[],
  bucket: number,
  seed: number,
): number {
...
function applyTargetTerritoryShares(
  ...
  for (let i = 0; i < rows.length; i++) {
    const target =
      i === rows.length - 1
        ? remaining
        : Math.max(1, Math.round(rows[i].share * bucketCount));
  ...
  const hqRooms = world.rooms
    ...
    .map((room) => ({
      room,
      owner: territoryRoomOwner(world, room.id),
  ...
  for (const row of rows) {
    for (let bucket = 0; bucket < bucketCount; bucket++) {
      candidates.push({
        owner: row.owner,
        bucket,
        score: ownerBucketScore(world, row.owner, anchors, bucket, seed),
      });
    }
  }
  candidates.sort((a, b) => a.score - b.score);
  for (const candidate of candidates) {
    if (ownerBuckets[candidate.bucket] !== UNOWNED_BUCKET) continue;
  ...
  for (let bucket = 0; bucket < bucketCount; bucket++) {
    if (ownerBuckets[bucket] !== UNOWNED_BUCKET) continue;
    ...
    for (const row of rows) {
      ...
      const overflow =
        used / cap +
        ownerBucketScore(world, row.owner, anchors, bucket, seed) * 1e-7;
  ...
  for (let by = 0; by < TERRITORY_BUCKET_SIDE; by++) {
    for (let bx = 0; bx < TERRITORY_BUCKET_SIDE; bx++) {
      const owner = ownerBuckets[
        by * TERRITORY_BUCKET_SIDE + bx
      ] as TerritoryOwner;
      for (let dy = 0; dy < TERRITORY_BUCKET_SIZE; dy++) {
        for (let dx = 0; dx < TERRITORY_BUCKET_SIZE; dx++) {
          const idx = world.idx(
            bx * TERRITORY_BUCKET_SIZE + dx,
            by * TERRITORY_BUCKET_SIZE + dy,
          );
...
function nearestZoneHqRoom(
  world: World,
  zone: { cx: number; cy: number; faction: TerritoryOwner },
): Room | undefined {
  ...
  for (const room of world.rooms) {
    ...
    if (
      d2 > bestD2 ||
      (d2 === bestD2 && best !== undefined && room.id >= best.id)
    )
      continue;
    best = room;
    bestD2 = d2;
...
export function syncZoneMetadataFromTerritory(
  world: World,
  ...
  for (const zoneId of ids) {
    ...
    for (
      let dy = -ZONE_SAMPLE_RADIUS;
      dy <= ZONE_SAMPLE_RADIUS;
      dy += ZONE_SAMPLE_STEP
    ) {
      for (
        let dx = -ZONE_SAMPLE_RADIUS;
        dx <= ZONE_SAMPLE_RADIUS;
        dx += ZONE_SAMPLE_STEP
      ) {
...
export function initializeCellTerritory(
  world: World,
  options: TerritoryInitializationOptions = {},
): void {
...
function actorCapturePressure(actor: Entity, owner: TerritoryOwner): boolean {
  if (
    occupationHasProfileTag(actor.occupation, "combat") ||
    actor.ai?.goal === AIGoal.HUNT
  )
    return true;
  let same = 0;
  let enemy = 0;
  getEntityIndex().queryRadiusCapped(
    actor.x,
    actor.y,
    CAPTURE_ACTOR_SCAN_RADIUS,
    captureQuery,
    ENTITY_MASK_NPC,
    CAPTURE_ACTOR_SCAN_CAP,
  );
  for (const other of captureQuery) {
    if (
      !other.alive ||
      other.type !== EntityType.NPC ||
...
export function updateTerritoryCapture(
  world: World,
  entities: Entity[],
  ...
  if (affectedZones.size > 0)
    syncZoneMetadataFromTerritory(world, affectedZones);
  return changedCells;
}
