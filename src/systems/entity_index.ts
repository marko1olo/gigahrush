...
export class EntityIndex {
  ...
  readonly anyMonsterByKind = new Map<number, Set<Entity>>();
  ...
  rebuild(
    ...
    for (let order = 0; order < entities.length; order++) {
      ...
      if (e.type === EntityType.MONSTER && e.monsterKind !== undefined) {
        let set = this.anyMonsterByKind.get(e.monsterKind);
        if (!set) {
          set = new Set();
          this.anyMonsterByKind.set(e.monsterKind, set);
        }
        set.add(e);
      }
    ...
    for (let dynamicOrder = 0; dynamicOrder < this.dynamicEntities.length; dynamicOrder++) {
      ...
      if (!e || !e.alive) {
        if (e) {
          ...
          if (e.type === EntityType.MONSTER && e.monsterKind !== undefined) {
            const set = this.anyMonsterByKind.get(e.monsterKind);
            if (set) set.delete(e);
          }
