...
function switchFloor(
  ...
  scheduleLoading(() => {
    ...
    const transitionTagsSet = new Set<string>(['floor', 'floor_transition', 'lift', route.activeInstance ? 'elevator_anomaly' : 'normal']);
...
function debugTeleportTo(target: DebugTeleportTarget): void {
  ...
  scheduleLoading(() => {
    ...
    const transitionTagsSet = new Set<string>(['floor', 'floor_transition', 'debug', target.spec ? 'procedural' : target.designFloorId ? 'design_floor' : 'story']);
