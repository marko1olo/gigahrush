...
function handleMobileHudTapControls(y: number, h: number): boolean {
  const { sy } = menuScale();
  const top = 34 * sy;
  const rowH = 12 * sy;
  const visible = controlsVisibleRows();
...
function handleMobileHudTapUiSettings(y: number, h: number): boolean {
  const { sy } = menuScale();
  const top = 34 * sy;
  const rowH = 12 * sy;
  const visible = uiSettingsVisibleRows();
...
function handleMobileHudTapMenu(x: number, y: number, w: number, h: number, sx: number, sy: number): boolean {
...
function handleMobileHudTapInventory(x: number, y: number, w: number, h: number, baseSx: number, baseSy: number): boolean {
...
function handleMobileHudTapCraftMenu(x: number, y: number, w: number, h: number): boolean {
...
function handleMobileHudTapQuests(x: number, y: number, w: number, h: number, sx: number, sy: number): boolean {
...
function handleMobileHudTapLog(y: number, h: number, sy: number): boolean {
...
function handleMobileHudTapDemos(x: number, y: number, w: number, h: number, sy: number): boolean {
  if (y > h - 28 * sy) {
    closeDemosMenu();
    updateMobileContext(true);
    return true;
...
function handleMobileHudTapContainerMenu(x: number, y: number, w: number, h: number): boolean {
  const container = world.containerById.get(state.containerMenuTarget);
  if (!container) {
    closeContainerMenu();
    return true;
...
function handleMobileHudTapNpcMenu(x: number, y: number, w: number, h: number, sx: number, sy: number): boolean {
  const npc = entities.find(e => e.id === state.npcMenuTarget);
  if (!npc) return true;
  if (state.npcMenuTab === 'main') {
...
function handleMobileHudTap(x: number, y: number): void {
  ...
  if (state.showControls) {
    if (handleMobileHudTapControls(y, h)) return;
  } else if (state.showUiSettings) {
    if (handleMobileHudTapUiSettings(y, h)) return;
  } else if (state.showMenu) {
    if (handleMobileHudTapMenu(x, y, w, h, sx, sy)) return;
  } else if (state.showInventory) {
    if (handleMobileHudTapInventory(x, y, w, h, baseSx, baseSy)) return;
  } else if (state.showCraftMenu) {
    if (handleMobileHudTapCraftMenu(x, y, w, h)) return;
  } else if (state.showQuests) {
    if (handleMobileHudTapQuests(x, y, w, h, sx, sy)) return;
  } else if (state.showLog) {
    if (handleMobileHudTapLog(y, h, sy)) return;
  } else if (state.showDemos) {
    if (handleMobileHudTapDemos(x, y, w, h, sy)) return;
  } else if (state.showFactions) {
    state.showFactions = false;
    syncPauseState();
  } else if (state.showContainerMenu) {
    if (handleMobileHudTapContainerMenu(x, y, w, h)) return;
  } else if (state.showNpcMenu) {
    if (handleMobileHudTapNpcMenu(x, y, w, h, sx, sy)) return;
  }
}
