...
function handleMobileControlsTap(y: number, h: number): void {
  const { sy } = menuScale();
  const top = 34 * sy;
  const rowH = 12 * sy;
  const visible = controlsVisibleRows();
...
function handleMobileUiSettingsTap(y: number, h: number): void {
  const { sy } = menuScale();
  const top = 34 * sy;
  const rowH = 12 * sy;
  const visible = uiSettingsVisibleRows();
...
function handleMobileMenuTap(x: number, y: number, w: number, h: number, sx: number, sy: number): void {
...
function handleMobileInventoryTap(x: number, y: number, w: number, h: number, baseSx: number, baseSy: number): void {
...
function handleMobileCraftMenuTap(x: number, y: number, w: number, h: number): void {
...
function handleMobileQuestsTap(x: number, y: number, w: number, h: number, sx: number, sy: number): void {
...
function handleMobileLogTap(y: number, h: number, sy: number): void {
...
function handleMobileDemosTap(x: number, y: number, w: number, h: number, sy: number): void {
  if (y > h - 28 * sy) {
    closeDemosMenu();
    updateMobileContext(true);
    return;
...
function handleMobileFactionsTap(): void {
  state.showFactions = false;
  syncPauseState();
}

function handleMobileContainerMenuTap(x: number, y: number, w: number, h: number): void {
  const container = world.containerById.get(state.containerMenuTarget);
  if (!container) {
    closeContainerMenu();
    return;
...
function handleMobileNpcMenuTap(x: number, y: number, w: number, h: number, sx: number, sy: number): void {
  const npc = entities.find(e => e.id === state.npcMenuTarget);
  if (!npc) return;
  if (state.npcMenuTab === 'main') {
...
function handleMobileHudTap(x: number, y: number): void {
  ...
  if (state.showControls) {
    handleMobileControlsTap(y, h);
  } else if (state.showUiSettings) {
    handleMobileUiSettingsTap(y, h);
  } else if (state.showMenu) {
    handleMobileMenuTap(x, y, w, h, sx, sy);
  } else if (state.showInventory) {
    handleMobileInventoryTap(x, y, w, h, baseSx, baseSy);
  } else if (state.showCraftMenu) {
    handleMobileCraftMenuTap(x, y, w, h);
  } else if (state.showQuests) {
    handleMobileQuestsTap(x, y, w, h, sx, sy);
  } else if (state.showLog) {
    handleMobileLogTap(y, h, sy);
  } else if (state.showDemos) {
    handleMobileDemosTap(x, y, w, h, sy);
  } else if (state.showFactions) {
    handleMobileFactionsTap();
  } else if (state.showContainerMenu) {
    handleMobileContainerMenuTap(x, y, w, h);
  } else if (state.showNpcMenu) {
    handleMobileNpcMenuTap(x, y, w, h, sx, sy);
  }
}
