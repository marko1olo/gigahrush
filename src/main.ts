...
function applyPlayerLook(actor: Entity, dt: number): boolean {
  // Mouse look
  if (input.mouse.locked) {
  ...
  if (actor.id === player.id && isRidingRailTrain(world, player)) return true;

  nudgeBlockedPlayerToFloor(actor);
  return false;
}

function calculatePlayerMovementVector(actor: Entity): { mx: number, my: number } {
...
function applyPlayerMovement(actor: Entity, mx: number, my: number, dt: number): { speed: number, moveMod: number, len: number } {
  const len = Math.sqrt(mx * mx + my * my);
  if (len <= 0) {
    return { speed: 0, moveMod: 0, len: 0 };
  }
...
function checkPlayerTeleport(actor: Entity): void {
...
function updatePlayerFootsteps(actor: Entity, speed: number, moveMod: number, len: number): void {
  if (len <= 0) return;
...
function movePlayer(dt: number): void {
  ...
  if (applyPlayerLook(actor, dt)) return;

  const { mx, my } = calculatePlayerMovementVector(actor);
  const { speed, moveMod, len } = applyPlayerMovement(actor, mx, my, dt);

  checkPlayerTeleport(actor);
  updatePlayerFootsteps(actor, speed, moveMod, len);
}
