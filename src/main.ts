...
function handleMobileControlsTap(y: number, h: number, sy: number): void {
  const top = 34 * sy;
  const rowH = 12 * sy;
  const visible = controlsVisibleRows();
...
function handleMobileUiSettingsTap(y: number, h: number, sy: number): void {
  const top = 34 * sy;
  const rowH = 12 * sy;
  const visible = uiSettingsVisibleRows();
  const relRow = Math.floor((y - top) / rowH);
...
function handleMobileHudTap(x: number, y: number): void {
  ...
  if (state.showControls) {
    handleMobileControlsTap(y, h, sy);
  } else if (state.showUiSettings) {
    handleMobileUiSettingsTap(y, h, sy);
