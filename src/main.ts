...
function processGameInput(now: number): boolean {
...
function updateGameSimulation(dt: number, frameDt: number): boolean {
...
function renderGame(now: number, rawDt: number, dt: number): void {
  checkRestart();
  updateMobileContext();
...
function gameLoop(now: number): void {
  ...
  if (processGameInput(now)) return;
  ...
  if (!state.paused && !state.gameOver) {
    if (updateGameSimulation(dt, frameDt)) return;
  }
