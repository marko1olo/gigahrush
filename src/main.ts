...
function handleControlsMenuTap(y: number, h: number): void {
  const { sy } = menuScale();
  const top = 34 * sy;
  const rowH = 12 * sy;
  const visible = controlsVisibleRows();
...
function handleUiSettingsMenuTap(y: number, h: number): void {
  const { sy } = menuScale();
  const top = 34 * sy;
  const rowH = 12 * sy;
  const visible = uiSettingsVisibleRows();
...
function handleGameMenuTap(x: number, y: number, w: number, h: number): void {
...
function handleInventoryTap(x: number, y: number, w: number, h: number, baseSx: number, baseSy: number): void {
...
function handleCraftMenuTap(x: number, y: number, w: number, h: number): void {
...
function handleQuestsMenuTap(x: number, y: number, w: number, h: number): void {
  const { sx, sy } = menuScale();
...
function handleLogMenuTap(y: number, h: number): void {
  const { sy } = menuScale();
...
function handleDemosMenuTap(x: number, y: number, w: number, h: number): void {
  const { sy } = menuScale();
  if (y > h - 28 * sy) {
    closeDemosMenu();
    updateMobileContext(true);
...
function handleContainerMenuTap(x: number, y: number, w: number, h: number): void {
  const container = world.containerById.get(state.containerMenuTarget);
  if (!container) {
    closeContainerMenu();
    return;
...
function handleNpcMenuTap(x: number, y: number, w: number, h: number): void {
  const { sx, sy } = menuScale();
  const npc = entities.find(e => e.id === state.npcMenuTarget);
  if (!npc) return;
  if (state.npcMenuTab === 'main') {
...
function handleMobileHudTap(x: number, y: number): void {
  ...
  if (state.showHelp) {
    ...
    handleControlsMenuTap(y, h);
  } else if (state.showUiSettings) {
    handleUiSettingsMenuTap(y, h);
  } else if (state.showMenu) {
    handleGameMenuTap(x, y, w, h);
  } else if (state.showInventory) {
    handleInventoryTap(x, y, w, h, baseSx, baseSy);
  } else if (state.showCraftMenu) {
    handleCraftMenuTap(x, y, w, h);
  } else if (state.showQuests) {
    handleQuestsMenuTap(x, y, w, h);
  } else if (state.showLog) {
    handleLogMenuTap(y, h);
  } else if (state.showDemos) {
    handleDemosMenuTap(x, y, w, h);
  } else if (state.showFactions) {
    state.showFactions = false;
    syncPauseState();
  } else if (state.showContainerMenu) {
    handleContainerMenuTap(x, y, w, h);
  } else if (state.showNpcMenu) {
    handleNpcMenuTap(x, y, w, h);
  }
}
