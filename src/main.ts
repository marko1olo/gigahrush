...
function handleMobileControlsTap(y: number, h: number, sy: number): boolean {
  const top = 34 * sy;
  const rowH = 12 * sy;
  const visible = controlsVisibleRows();
...
function handleMobileUiSettingsTap(y: number, h: number, sy: number): boolean {
  const top = 34 * sy;
  const rowH = 12 * sy;
  const visible = uiSettingsVisibleRows();
  const relRow = Math.floor((y - top) / rowH);
...
function handleMobileMenuTap(x: number, y: number, w: number, h: number, sx: number, sy: number): boolean {
...
function handleMobileInventoryTap(x: number, y: number, w: number, h: number, baseSx: number, baseSy: number): boolean {
...
function handleMobileCraftMenuTap(x: number, y: number, w: number, h: number): boolean {
...
function handleMobileQuestsTap(x: number, y: number, w: number, h: number, sx: number, sy: number): boolean {
...
function handleMobileLogTap(y: number, h: number, sy: number): boolean {
...
function handleMobileDemosTap(x: number, y: number, w: number, h: number, sy: number): boolean {
  if (y > h - 28 * sy) {
    closeDemosMenu();
    updateMobileContext(true);
    return true;
...
function handleMobileContainerMenuTap(x: number, y: number, w: number, h: number): boolean {
  const container = world.containerById.get(state.containerMenuTarget);
  if (!container) {
    closeContainerMenu();
    return true;
...
function handleMobileNpcMenuTap(x: number, y: number, w: number, h: number, sx: number, sy: number): boolean {
  const npc = entities.find(e => e.id === state.npcMenuTarget);
  if (!npc) return true;
  if (state.npcMenuTab === 'main') {
...
function handleMobileHudTap(x: number, y: number): void {
  ...
  if (state.showControls) {
    if (handleMobileControlsTap(y, h, sy)) return;
  } else if (state.showUiSettings) {
    if (handleMobileUiSettingsTap(y, h, sy)) return;
  } else if (state.showMenu) {
    if (handleMobileMenuTap(x, y, w, h, sx, sy)) return;
  } else if (state.showInventory) {
    if (handleMobileInventoryTap(x, y, w, h, baseSx, baseSy)) return;
  } else if (state.showCraftMenu) {
    if (handleMobileCraftMenuTap(x, y, w, h)) return;
  } else if (state.showQuests) {
    if (handleMobileQuestsTap(x, y, w, h, sx, sy)) return;
  } else if (state.showLog) {
    if (handleMobileLogTap(y, h, sy)) return;
  } else if (state.showDemos) {
    if (handleMobileDemosTap(x, y, w, h, sy)) return;
  } else if (state.showFactions) {
    state.showFactions = false;
    syncPauseState();
  } else if (state.showContainerMenu) {
    if (handleMobileContainerMenuTap(x, y, w, h)) return;
  } else if (state.showNpcMenu) {
    if (handleMobileNpcMenuTap(x, y, w, h, sx, sy)) return;
  }
}
