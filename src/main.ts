...
function handleDeferredLoading(now: number): boolean {
...
function updateActiveSimulation(dt: number, frameDt: number): boolean {
  ...
  if (state.sleeping && player.needs) {
    player.needs.sleep = Math.min(100, player.needs.sleep + (100 / 300) * dt);
...
function updateGameOverSimulation(dt: number, frameDt: number): boolean {
  state.time += dt;
  state.tick++;
  state.clock.totalMinutes += dt;
  const totalMins = Math.floor(state.clock.totalMinutes);
...
function renderGameFrame(dt: number, _rawDt: number, currentFps: number): void {
...
function gameLoop(now: number): void {
  ...
  if (!state.paused && !state.gameOver) {
    if (updateActiveSimulation(dt, frameDt)) return;
  }

  if (!state.paused && state.gameOver) {
    if (updateGameOverSimulation(dt, frameDt)) return;
  }
  ...
  // ── Render ───────────────────────────────────────────────
  renderGameFrame(dt, rawDt, currentFps);
