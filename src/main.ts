...
function handleMobileControlsMenuTap(y: number, h: number): void {
  const { sy } = menuScale();
  const top = 34 * sy;
  const rowH = 12 * sy;
  const visible = controlsVisibleRows();
...
function handleMobileUiSettingsMenuTap(y: number, h: number): void {
  const { sy } = menuScale();
  const top = 34 * sy;
  const rowH = 12 * sy;
  const visible = uiSettingsVisibleRows();
...
function handleMobileGameMenuTap(x: number, y: number, w: number, h: number, sx: number, sy: number): void {
...
function handleMobileInventoryMenuTap(x: number, y: number, w: number, h: number, baseSx: number, baseSy: number): void {
...
function handleMobileQuestsMenuTap(x: number, y: number, w: number, h: number, sx: number, sy: number): void {
...
function handleMobileLogMenuTap(y: number, h: number, sy: number): void {
...
function handleMobileDemosMenuTap(x: number, y: number, w: number, h: number, sy: number): void {
  if (y > h - 28 * sy) {
    closeDemosMenu();
    updateMobileContext(true);
    return;
...
function handleMobileHudTap(x: number, y: number): void {
  ...
  if (state.showControls) {
    handleMobileControlsMenuTap(y, h);
  } else if (state.showUiSettings) {
    handleMobileUiSettingsMenuTap(y, h);
  } else if (state.showMenu) {
    handleMobileGameMenuTap(x, y, w, h, sx, sy);
  } else if (state.showInventory) {
    handleMobileInventoryMenuTap(x, y, w, h, baseSx, baseSy);
  } else if (state.showCraftMenu) {
    handleMobileCraftMenuTap(x, y, w, h);
  } else if (state.showQuests) {
    handleMobileQuestsMenuTap(x, y, w, h, sx, sy);
  } else if (state.showLog) {
    handleMobileLogMenuTap(y, h, sy);
  } else if (state.showDemos) {
    handleMobileDemosMenuTap(x, y, w, h, sy);
