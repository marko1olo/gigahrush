...
export function drawUiSettingsMenu(
  ...
  ctx.fillStyle = '#6cf';
  ctx.font = getUiFont(11 * sy, false);
  drawShadowText(ctx, view === 'graphics' ? 'НАСТРОЙКИ ГРАФИКИ' : 'НАСТРОЙКИ ИНТЕРФЕЙСА', 12 * sx, 12 * sy);
  ...
  drawShadowText(ctx, view === 'graphics' ? 'НАСТРОЙКА' : 'ПРЕСЕТ / ЭЛЕМЕНТ', x + groupW + 18 * sx, top - 7 * sy);
  drawShadowText(ctx, view === 'graphics' ? 'ЗНАЧЕНИЕ' : 'СТАТУС', x + groupW + labelW + 20 * sx, top - 7 * sy);
  ...
  for (let row = 0; row < visible; row++) {
    ...
    if (isReset) {
      ctx.fillStyle = '#b98';
      drawShadowText(ctx, item.group, x + 14 * sx, textY);
      ctx.fillStyle = isSel ? '#ffe7ad' : '#d7c38a';
      drawShadowText(ctx, fitTextStable(ctx, item.label, labelW - 8 * sx), x + groupW + 18 * sx, textY);
      ctx.fillStyle = '#fd6';
      drawShadowText(ctx, 'ENTER', x + groupW + labelW + 20 * sx, textY);
    } else if (isPreset) {
      ctx.fillStyle = '#b98';
      drawShadowText(ctx, 'ПРЕСЕТ', x + 14 * sx, textY);
      ctx.fillStyle = isSel ? '#ffe7ad' : '#d7c38a';
      drawShadowText(ctx, fitTextStable(ctx, `${item.preset.label}: ${item.preset.hint}`, labelW - 8 * sx), x + groupW + 18 * sx, textY);
      ctx.fillStyle = enabled ? '#fd6' : '#789';
      drawShadowText(ctx, enabled ? 'ВЫБРАН' : 'ПРИМ', x + groupW + labelW + 20 * sx, textY);
    } else if (item.kind === 'element') {
      const element = item.element;
      ctx.fillStyle = '#689';
      drawShadowText(ctx, fitTextStable(ctx, element.group, groupW - 10 * sx), x + 14 * sx, textY);
      ctx.fillStyle = isSel ? '#dff' : '#9bb';
      drawShadowText(ctx, fitTextStable(ctx, element.label, labelW - 8 * sx), x + groupW + 18 * sx, textY);
      ctx.fillStyle = element.locked ? '#fd6' : enabled ? '#8f8' : '#b66';
      drawShadowText(ctx, element.locked ? 'ВСЕГДА' : enabled ? 'ВКЛ' : 'ВЫКЛ', x + groupW + labelW + 20 * sx, textY);
    } else if (item.kind === 'auto_pickup') {
      ctx.fillStyle = '#689';
      drawShadowText(ctx, fitTextStable(ctx, item.group, groupW - 10 * sx), x + 14 * sx, textY);
      ctx.fillStyle = isSel ? '#dff' : '#9bb';
      drawShadowText(ctx, fitTextStable(ctx, item.label, labelW - 8 * sx), x + groupW + 18 * sx, textY);
      ctx.fillStyle = enabled ? '#8f8' : '#fc8';
      drawShadowText(ctx, enabled ? 'ВКЛ' : 'ВЫКЛ', x + groupW + labelW + 20 * sx, textY);
    } else {
      ctx.fillStyle = '#689';
      drawShadowText(ctx, fitTextStable(ctx, item.group, groupW - 10 * sx), x + 14 * sx, textY);
      ctx.fillStyle = isSel ? '#dff' : '#9bb';
      ...
      ctx.fillStyle = '#fd6';
      drawShadowText(ctx,
        view === 'graphics' ? graphicsValue(item.kind) : `${Math.round(mobileLookSensitivity() * 100)}%`, x + groupW + labelW + 20 * sx, textY,
      );
    }
  }
