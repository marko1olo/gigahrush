...
export function renderSceneGL(
  ...
  if (getCritterRenderEnabled(currentFps)) {
    renderCrittersGL(px, py, pAngle, pPitch, camHeight, fogDensity, purpleFog, fogRgb, planeLen);
  }
...
function renderCrittersGL(
  ...
  for (let i = 0; i < CRITTERS_POOL.length; i++) {
    ...
    // Set critter size based on type (2-4px)
    let baseSize = 2;
    let r = 100, g = 60, b = 20; // Roach (brown)
    if (c.type === 'rat') {
      baseSize = 4;
      r = 50; g = 50; b = 50; // Rat (dark grey)
    } else if (c.type === 'fly') {
      baseSize = 2;
      r = 10; g = 10; b = 10; // Fly (black)
    }

    const screenSize = Math.min(
      PARTICLE_MAX_SCREEN_SIZE,
      (SCR_H / tyf) * PARTICLE_WORLD_SCREEN_SCALE * Math.max(0.5, baseSize),
    );
    if (screenSize < PARTICLE_MIN_SCREEN_SIZE) continue;
    ...
    instanceData[di + 2] = screenSize;
    instanceData[di + 3] = normDepth;
    colorData[di] = (r / 255) * invFogF + fogR * fogF;
    colorData[di + 1] = (g / 255) * invFogF + fogG * fogF;
    colorData[di + 2] = (b / 255) * invFogF + fogB * fogF;
    colorData[di + 3] = alpha;
    visibleCount++;
  }
