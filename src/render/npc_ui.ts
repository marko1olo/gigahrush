...
export function drawNpcMenu(
  ...
  drawShadowText(ctx, fitText(ctx, `${fName} · ${oName}`, pw - 16 * sx), px + 8 * sx + fj.dx, py + 22 * sy + fj.dy);

  if (state.npcMenuTab === 'main') {
    const items = getNpcMenuOptions({ state, player, npc, entities });
    ctx.font = getUiFont(8.6 * sy, false);
    for (let i = 0; i < items.length; i++) {
      ...
      drawShadowText(ctx, `${selected ? '▶ ' : '  '}${items[i].label}`, px + 16 * sx + mj.dx, yy + mj.dy);
    }
    ctx.fillStyle = '#456';
    ctx.font = getUiFont(6.8 * sy, false);
    drawShadowText(ctx, fitText(ctx, `${controlBindingLabel('menuUp')}/${controlBindingLabel('menuDown')} выбор  |  ${controlHint('gameMenu')} выбрать  |  ${menuCloseHint()} закрыть`, pw - 16 * sx), px + 8 * sx, py + ph - 11 * sy);
    ...
    ctx.fillStyle = '#555';
    ctx.font = getUiFont(6.8 * sy, false);
    drawShadowText(ctx, `${controlHint('gameMenu')} назад  |  ${menuCloseHint()} закрыть`, px + 8 * sx, py + ph - 11 * sy);
    ...
    if (total === 0) {
      ctx.fillStyle = '#888';
      drawShadowText(ctx, 'Активных заданий нет.', px + 8 * sx, py + 40 * sy);
    } else {
      ...
      ctx.fillStyle = '#888';
      ctx.font = getUiFont(6.8 * sy, false);
      drawShadowText(ctx, `${page + 1} / ${total}`, px + pw - 40 * sx, py + 10 * sy);
      // Quest giver
      ctx.fillStyle = '#8af';
      ctx.font = getUiFont(7.8 * sy, false);
      drawShadowText(ctx, fitText(ctx, `От: ${q.giverName ?? '???'}`, pw - 16 * sx), px + 8 * sx, py + 38 * sy);
    ...
    drawShadowText(ctx, fitText(ctx, hint, pw - 16 * sx), px + 8 * sx, py + ph - 11 * sy);
    ...
    ctx.font = getUiFont(9.2 * sy, false);
    ...
    let ly = py + 56 * sy;
    const maxW = pw - 16 * sx;
    drawShadowText(ctx, fitText(ctx, snapshot.npcName || npc.name || 'NPC', maxW), px + 8 * sx, ly);
    ly += 14 * sy;
    if (snapshot.stakeRubles !== undefined) {
      ctx.fillStyle = '#ee4';
      drawShadowText(ctx, fitText(ctx, `Ставка: ₽${snapshot.stakeRubles}`, maxW), px + 8 * sx, ly);
      ly += 12 * sy;
    }
    if (snapshot.priceRubles !== undefined) {
      ctx.fillStyle = '#ee4';
      drawShadowText(ctx, fitText(ctx, `Цена: ₽${snapshot.priceRubles}`, maxW), px + 8 * sx, ly);
      ly += 12 * sy;
    }
    ctx.fillStyle = '#ccc';
    ctx.font = getUiFont(7.8 * sy, false);
    ...
    drawShadowText(ctx, 'ТОРГ', cw / 2, 10 * sy);
    ctx.textAlign = 'left';

    // ── Headers with money ──
    ctx.font = getUiFont(7.1 * sy, false);
    ctx.fillStyle = '#ee4';
    drawShadowText(ctx, `Вы: ₽${player.money ?? 0}`, startX, startY - 8 * sy);
    ctx.fillStyle = '#cc9';
    drawShadowText(ctx, fitText(ctx, `Вы отдаете · ${deal.creditValue}₽`, gridTotal), playerOfferX, startY - 8 * sy);
    ctx.fillStyle = '#9cf';
    drawShadowText(ctx, fitText(ctx, `Вы берете · ${deal.fullPrice}₽`, gridTotal), npcOfferX, startY - 8 * sy);
    ctx.fillStyle = '#8cf';
    drawShadowText(ctx,
      ...
      for (let row = 0; row < gridRows; row++) {
        for (let col = 0; col < gridCols; col++) {
          ...
          if (idx < inv.length) {
            ...
            ctx.font = getUiFont(Math.max(4, 3.5 * sy), false);
            ...
            drawShadowText(ctx, fitText(ctx, roleLabel, Math.max(1, innerW - questW)), cx + padX, topY);
            if (questLabel) {
              ctx.fillStyle = questItemStateColor(price.questState);
              ctx.textAlign = 'right';
              drawShadowText(ctx, fitText(ctx, questLabel, questW), cx + cellSz - padX, topY);
              ctx.textAlign = 'left';
            }
            ...
            ctx.font = getUiFont(Math.max(4, 4.05 * sy), false);
            ...
            drawShadowText(ctx, fitText(ctx, price.text, Math.max(1, innerW - countW)), cx + padX, bottomY);
            if (countText) {
              ctx.textAlign = 'right';
              ctx.fillStyle = staged > 0 ? '#fc4' : '#8a8';
              drawShadowText(ctx, fitText(ctx, countText, countW), cx + cellSz - padX, bottomY);
              ctx.textAlign = 'left';
            }
            ctx.restore();
          }
    ...
    drawShadowText(ctx, fitText(ctx, summaryLine, dealW - 8 * sx), dealX + dealW / 2, dealY + dealH + 7.2 * sy);
    ctx.textAlign = 'left';
    ...
    if (curIdx < curInv.length) {
      ...
      if (def) {
        ...
        drawShadowText(ctx, fitText(ctx, `${def.name} ×${item.count}`, descW), cw / 2, descY);
        ctx.fillStyle = '#888';
        ctx.font = getUiFont(6.3 * sy, false);
        ...
        if (state.tradeSide === 'npc') {
          drawShadowText(ctx, fitText(ctx, `${controlBindingLabel('gameMenu')} запросить: ${price.line}`, descW), cw / 2, actionY);
          ctx.fillStyle = price.scarcityColor;
          drawShadowText(ctx, fitText(ctx, price.detail, descW), cw / 2, actionY + 8 * sy);
        } else if (state.tradeSide === 'player_offer') {
          drawShadowText(ctx, fitText(ctx, `${controlBindingLabel('gameMenu')} убрать единицу из того, что отдаете`, descW), cw / 2, actionY);
        } else if (state.tradeSide === 'npc_offer') {
          drawShadowText(ctx, fitText(ctx, `${controlBindingLabel('gameMenu')} убрать единицу из того, что берете`, descW), cw / 2, actionY);
        } else {
          drawShadowText(ctx, fitText(ctx, `${controlBindingLabel('gameMenu')} предложить: ${price.line}`, descW), cw / 2, actionY);
          ctx.fillStyle = price.scarcityColor;
          drawShadowText(ctx, fitText(ctx, price.detail, descW), cw / 2, actionY + 8 * sy);
        }
      }
      ...
      drawShadowText(ctx, fitText(ctx, canDeal ? `${controlBindingLabel('gameMenu')} подтвердить сделку` : 'Сделка пока невозможна', Math.min(cw - 16 * sx, totalW)), cw / 2, descY + 6 * sy);
    } else {
      ctx.fillStyle = '#555';
      ctx.font = getUiFont(6.2 * sy, false);
      drawShadowText(ctx, 'Пусто', cw / 2, descY + 6 * sy);
    }
    ctx.textAlign = 'left';
    ...
    drawShadowText(ctx, fitText(ctx, `${controlBindingLabel('gameMenu')} положить/убрать/обмен`, hintW), cw - 8 * sx, ch - 16 * sy);
    drawShadowText(ctx, fitText(ctx, `${menuCloseHint()} назад`, hintW), cw - 8 * sx, ch - 8 * sy);
    ctx.textAlign = 'left';
