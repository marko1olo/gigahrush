# Items 178: pump_impeller

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `pump_impeller` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:91 (Trade, production, contraband, electronics and resident goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Water/pump repair |
| reachability target | Production/Maintenance |
| implementation note | Use with pump_passport. |
...
- src/data/item_packs/pump_impeller.ts
...
- Add or improve only `pump_impeller`.
...
- Whether `pump_impeller` was added, improved, merged into an existing id or rejected.
