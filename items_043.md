...
Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `ak47` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:93 (Liquidator weapons, ammo and combat roles)
...
| gameplay role | Existing automatic rifle |
| reachability target | Current item |
| implementation note | Keep as old-world analogue; don't duplicate if Ералаш uses same role. |
...
- src/data/item_packs/ak47.ts plus src/data/weapon_packs/ak47.ts for weapons
...
- Whether `ak47` was added, improved, merged into an existing id or rejected.
