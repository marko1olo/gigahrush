# Items 024: liquidator_flashlamp

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `liquidator_flashlamp` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:92 (Liquidator cleanup, PPE and samosbor field gear)
...
| Russian name | Переносной прожектор |
| mode | new |
| intended type | TOOL |
| gameplay role | Heavy light source, slower than flashlight |
| reachability target | HQ/production |
| implementation note | Reuse light/tool patterns if possible. |
...
- src/data/item_packs/liquidator_flashlamp.ts
...
- Add or improve only `liquidator_flashlamp`.
...
- Whether `liquidator_flashlamp` was added, improved, merged into an existing id or rejected.
