# Items 155: protein_mold_cake

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `protein_mold_cake` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:111 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `protein_mold_cake` |
| Russian name | Плесневой белковый брикет |
| mode | new |
| intended type | FOOD |
| gameplay role | Mold-origin food |
| reachability target | Kitchen/fungal zones |
| implementation note | Ties setting to food without duplicate. |
...
- src/data/item_packs/protein_mold_cake.ts
...
- Add or improve only `protein_mold_cake`.
...
- Whether `protein_mold_cake` was added, improved, merged into an existing id or rejected.
