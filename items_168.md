# Items 168: junior_tech_case

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `junior_tech_case` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:81 (Trade, production, contraband, electronics and resident goods)
...
| --- | --- |
| id | `junior_tech_case` |
| Russian name | Корпус «Юный техник» |
| mode | new |
| intended type | MISC |
| gameplay role | Electronics casing |
| reachability target | Cabinets/vending |
| implementation note | Source item, map electronics. |
...
- src/data/item_packs/junior_tech_case.ts
...
- Add or improve only `junior_tech_case`.
...
- Whether `junior_tech_case` was added, improved, merged into an existing id or rejected.
