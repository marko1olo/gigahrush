...
### Контекст задачи
Интеграция экономической системы ставок: списание денег, расчёт коэффициента, выплата выигрыша.

### Конкретные файлы и паттерны
- **`src/systems/inventory.ts`**: `Entity.money` — кошелёк игрока. `transferMoney()` для транзакций.
- **`src/render/arena_ui.ts`** (от marx_24): Добавьте ввод ставки (slider или preset кнопки: 10, 50, 100, 500 рублей).
- **Коэффициент**: Рассчитывайте из сравнения `level`, `hp`, наличия оружия у бойцов. Фаворит = коэффициент 1.5x, аутсайдер = 3x.
- **`src/systems/events.ts`**: Публикуйте `'arena_bet_placed'`, `'arena_bet_won'`, `'arena_bet_lost'`.
- **Без save**: Ставки — транзиентное состояние, не нужно добавлять в save payload. Деньги уже сохраняются через `Entity.money`.
