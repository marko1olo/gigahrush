# Items 115: water_reservoir_quota

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `water_reservoir_quota` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:107 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Water ration gate |
| reachability target | Office/kitchen |
| implementation note | Pair with water economy. |
...
- src/data/item_packs/water_reservoir_quota.ts
...
- Add or improve only `water_reservoir_quota`.
...
- Whether `water_reservoir_quota` was added, improved, merged into an existing id or rejected.
