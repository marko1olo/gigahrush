...
Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `rpl23_lmg` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:105 (Liquidator weapons, ammo and combat roles)
...
| gameplay role | Squad LMG |
| reachability target | Deep HQ/engineer route |
| implementation note | High ammo burn, heavy value. |
...
- src/data/item_packs/rpl23_lmg.ts plus src/data/weapon_packs/rpl23_lmg.ts for weapons
...
- Whether `rpl23_lmg` was added, improved, merged into an existing id or rejected.
