# Items 029: hermetic_tape

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `hermetic_tape` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:97 (Liquidator cleanup, PPE and samosbor field gear)
...
| mode | new |
| intended type | MISC |
| gameplay role | Fast temporary seal |
| reachability target | Storage/medical |
| implementation note | Stackable consumable. |
...
- src/data/item_packs/hermetic_tape.ts
...
- Add or improve only `hermetic_tape`.
...
- Whether `hermetic_tape` was added, improved, merged into an existing id or rejected.
