# Items 028: slime_scraper

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `slime_scraper` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:96 (Liquidator cleanup, PPE and samosbor field gear)
...
| --- | --- |
| id | `slime_scraper` |
| Russian name | Скребок для слизи |
| mode | new |
| intended type | TOOL |
| gameplay role | Low-tech cleanup |
| reachability target | Storage/production |
| implementation note | Durability tool; not a better weapon. |
...
- src/data/item_packs/slime_scraper.ts
...
- Add or improve only `slime_scraper`.
...
- Whether `slime_scraper` was added, improved, merged into an existing id or rejected.
