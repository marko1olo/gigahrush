# Items 086: ammo_coupon_shells

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `ammo_coupon_shells` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:78 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Legal shotgun ammo access |
| reachability target | Office/HQ |
| implementation note | Converts to ammo_shells through handler/contract. |
...
- src/data/item_packs/ammo_coupon_shells.ts
...
- Add or improve only `ammo_coupon_shells`.
...
- Whether `ammo_coupon_shells` was added, improved, merged into an existing id or rejected.
