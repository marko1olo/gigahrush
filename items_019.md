# Items 019: body_bag_roll

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `body_bag_roll` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:87 (Liquidator cleanup, PPE and samosbor field gear)
...
| mode | new |
| intended type | MISC |
| gameplay role | Aftermath contract item |
| reachability target | HQ/medical |
| implementation note | Turns death cleanup into decision: report vs hide. |
...
- src/data/item_packs/body_bag_roll.ts
...
- Add or improve only `body_bag_roll`.
...
- Whether `body_bag_roll` was added, improved, merged into an existing id or rejected.
