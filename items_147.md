# Items 147: lice_shampoo

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `lice_shampoo` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:103 (NII sampleware, slime, medicine, food and anomaly loot)
...
| --- | --- |
| id | `lice_shampoo` |
| Russian name | Шампунь от вшей |
| mode | new |
| intended type | MISC |
| gameplay role | Hygiene/parasite item |
| reachability target | Vending/medical |
| implementation note | Mostly trade/status future. |
...
- src/data/item_packs/lice_shampoo.ts
...
- Add or improve only `lice_shampoo`.
...
- Whether `lice_shampoo` was added, improved, merged into an existing id or rejected.
