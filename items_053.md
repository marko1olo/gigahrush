# Items 053: pushkin_shotgun

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `pushkin_shotgun` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:103 (Liquidator weapons, ammo and combat roles)
...
| gameplay role | Tactical shotgun shell platform |
| reachability target | Rare |
| implementation note | Only after ammo-selection decision. |
...
- src/data/item_packs/pushkin_shotgun.ts plus src/data/weapon_packs/pushkin_shotgun.ts for weapons
...
- Add or improve only `pushkin_shotgun`.
...
- Whether `pushkin_shotgun` was added, improved, merged into an existing id or rejected.
