# Items 112: samosbor_alarm_schedule

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `samosbor_alarm_schedule` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:104 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | False/real alarm document |
| reachability target | Office/HQ |
| implementation note | Must not predict samosbor globally unless system supports. |
...
- src/data/item_packs/samosbor_alarm_schedule.ts
...
- Add or improve only `samosbor_alarm_schedule`.
...
- Whether `samosbor_alarm_schedule` was added, improved, merged into an existing id or rejected.
