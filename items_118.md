# Items 118: empty_sample_jar

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `empty_sample_jar` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:74 (NII sampleware, slime, medicine, food and anomaly loot)
...
| mode | new |
| intended type | MISC |
| gameplay role | Cheap unofficial sampleware |
| reachability target | Medical/storage |
| implementation note | Lower value, breaks legal handoff. |
...
- src/data/item_packs/empty_sample_jar.ts
...
- Add or improve only `empty_sample_jar`.
...
- Whether `empty_sample_jar` was added, improved, merged into an existing id or rejected.
