# Items 144: technical_spirit

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `technical_spirit` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:100 (NII sampleware, slime, medicine, food and anomaly loot)
...
| mode | new |
| intended type | MISC |
| gameplay role | Sterilization/fuel/contraband |
| reachability target | Medical/production |
| implementation note | Coordinate with items_5 for brewing. |
...
- src/data/item_packs/technical_spirit.ts
...
- Add or improve only `technical_spirit`.
...
- Whether `technical_spirit` was added, improved, merged into an existing id or rejected.
