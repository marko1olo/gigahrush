# Items 114: shelter_seat_forgery

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `shelter_seat_forgery` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:106 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Forged shelter access |
| reachability target | Smoking/office |
| implementation note | Legal/forged pair. |
...
- src/data/item_packs/shelter_seat_forgery.ts
...
- Add or improve only `shelter_seat_forgery`.
...
- Whether `shelter_seat_forgery` was added, improved, merged into an existing id or rejected.
