# Items 129: slime_calcified_chip

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `slime_calcified_chip` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:85 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `slime_calcified_chip` |
| Russian name | Окаменевший скол слизи |
| mode | new |
| intended type | MISC |
| gameplay role | Mature/dead slime material |
| reachability target | Aftermath/deep route |
| implementation note | Good factory/NII input. |
...
- src/data/item_packs/slime_calcified_chip.ts
...
- Add or improve only `slime_calcified_chip`.
...
- Whether `slime_calcified_chip` was added, improved, merged into an existing id or rejected.
