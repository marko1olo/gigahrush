# Items 177: water_filter_regulator

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `water_filter_regulator` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:90 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `water_filter_regulator` |
| Russian name | Регулятор фильтра воды |
| mode | new |
| intended type | MISC |
| gameplay role | Water system repair |
| reachability target | Kitchens/Maintenance |
| implementation note | Source-backed by ventilation/water system. |
...
- src/data/item_packs/water_filter_regulator.ts
...
- Add or improve only `water_filter_regulator`.
...
- Whether `water_filter_regulator` was added, improved, merged into an existing id or rejected.
