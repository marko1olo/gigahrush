# Items 068: breach_charge

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `breach_charge` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:118 (Liquidator weapons, ammo and combat roles)
...
| --- | --- |
| id | `breach_charge` |
| Russian name | Пробивной заряд |
| mode | new |
| intended type | WEAPON |
| gameplay role | Door/wall/biomass breach |
| reachability target | Engineer stash/factory |
| implementation note | Bounded use only, publish event. |
...
- src/data/item_packs/breach_charge.ts plus src/data/weapon_packs/breach_charge.ts for weapons
...
- Add or improve only `breach_charge`.
...
- Whether `breach_charge` was added, improved, merged into an existing id or rejected.
