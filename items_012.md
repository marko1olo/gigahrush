# Items 012: sealant_tube

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `sealant_tube` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:80 (Liquidator cleanup, PPE and samosbor field gear)
...
| --- | --- |
| id | `sealant_tube` |
| Russian name | Тюбик герметика |
| mode | improve |
| intended type | MISC |
| gameplay role | Cheap repair consumable |
| reachability target | Existing rooms |
| implementation note | Make visible in cleanup recipe/contract list. |
...
- src/data/item_packs/sealant_tube.ts
...
- Add or improve only `sealant_tube`.
...
- Whether `sealant_tube` was added, improved, merged into an existing id or rejected.
