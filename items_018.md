# Items 018: cleanup_tongs

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `cleanup_tongs` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:86 (Liquidator cleanup, PPE and samosbor field gear)
...
| mode | new |
| intended type | TOOL |
| gameplay role | Safer sample pickup / corpse handling |
| reachability target | Medical/HQ |
| implementation note | Durability-based tool. |
...
- src/data/item_packs/cleanup_tongs.ts
...
- Add or improve only `cleanup_tongs`.
...
- Whether `cleanup_tongs` was added, improved, merged into an existing id or rejected.
