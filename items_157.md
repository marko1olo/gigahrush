# Items 157: mutant_tissue_sample

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `mutant_tissue_sample` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:113 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `mutant_tissue_sample` |
| Russian name | Образец ткани твари |
| mode | new |
| intended type | MISC |
| gameplay role | Ecology proof |
| reachability target | Monster drops/contracts |
| implementation note | Coordinate with monster ecology. |
...
- src/data/item_packs/mutant_tissue_sample.ts
...
- Add or improve only `mutant_tissue_sample`.
...
- Whether `mutant_tissue_sample` was added, improved, merged into an existing id or rejected.
