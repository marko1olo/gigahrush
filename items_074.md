# Items 074: ammo_12g_incendiary

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `ammo_12g_incendiary` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:124 (Liquidator weapons, ammo and combat roles)
...
| Field | Value |
| --- | --- |
| id | `ammo_12g_incendiary` |
| Russian name | Зажигательная дробь |
| mode | new |
| intended type | AMMO |
| gameplay role | Slime/fungus counterplay |
| reachability target | Rare HQ/factory |
| implementation note | Maybe map as fuel+shell recipe. |
...
- src/data/item_packs/ammo_12g_incendiary.ts plus src/data/weapon_packs/ammo_12g_incendiary.ts for weapons
...
- Add or improve only `ammo_12g_incendiary`.
...
- Ammo must be `ItemType.AMMO`, `spawnRooms: []`, `spawnW: 0`, explicitly resource-mapped and reachable through a non-generic path.
- Do not add specialty ammo selection unless the orchestrator has already merged a generic selection path.
...
- Whether `ammo_12g_incendiary` was added, improved, merged into an existing id or rejected.
