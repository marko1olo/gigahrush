# Items 004: ip4_gasmask

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `ip4_gasmask` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:72 (Liquidator cleanup, PPE and samosbor field gear)
...
| --- | --- |
| id | `ip4_gasmask` |
| Russian name | Противогаз ИП-4 |
| mode | new |
| intended type | TOOL |
| gameplay role | Respiratory PPE token |
| reachability target | HQ, rare trade, liquidator issue |
| implementation note | Use durability/filter state if generic handler exists. |
...
- src/data/item_packs/ip4_gasmask.ts
...
- Add or improve only `ip4_gasmask`.
...
- Whether `ip4_gasmask` was added, improved, merged into an existing id or rejected.
