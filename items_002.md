# Items 002: rusty_rake

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `rusty_rake` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:70 (Liquidator cleanup, PPE and samosbor field gear)
- items_2.md:82 (Liquidator weapons, ammo and combat roles)
...
| gameplay role | Bad starter cleanup weapon |
| reachability target | Storage, abandoned cells |
| implementation note | Upgrade input for liquidator_rake. |
...
- items_1.md:70: Bad starter cleanup weapon; Upgrade input for `liquidator_rake`.
- items_2.md:82: Bad early reach weapon; Upgrade or trade into official rake.
...
- src/data/item_packs/rusty_rake.ts
- src/data/item_packs/rusty_rake.ts plus src/data/weapon_packs/rusty_rake.ts for weapons
...
- Add or improve only `rusty_rake`.
...
- Whether `rusty_rake` was added, improved, merged into an existing id or rejected.
