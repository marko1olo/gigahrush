# Items 170: keyboard_unit

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `keyboard_unit` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:83 (Trade, production, contraband, electronics and resident goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Terminal hack/scrap part |
| reachability target | Office/living |
| implementation note | Use electronics resource. |
...
- src/data/item_packs/keyboard_unit.ts
...
- Add or improve only `keyboard_unit`.
...
- Whether `keyboard_unit` was added, improved, merged into an existing id or rejected.
