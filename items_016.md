# Items 016: zinc_slime_bucket

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `zinc_slime_bucket` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:84 (Liquidator cleanup, PPE and samosbor field gear)
...
| Field | Value |
| --- | --- |
| id | `zinc_slime_bucket` |
| Russian name | Цинковое ведро для слизи |
| mode | new |
| intended type | MISC |
| gameplay role | Cleanup proof, sample transport |
| reachability target | Samosbor aftermath, Maintenance |
| implementation note | Keep stack 1. |
...
- src/data/item_packs/zinc_slime_bucket.ts
...
- Add or improve only `zinc_slime_bucket`.
...
- Whether `zinc_slime_bucket` was added, improved, merged into an existing id or rejected.
