# Items 145: cotton_wool

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `cotton_wool` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:101 (NII sampleware, slime, medicine, food and anomaly loot)
...
| mode | new |
| intended type | MISC |
| gameplay role | Medical/filter component |
| reachability target | Medical/storage |
| implementation note | Resource medicine or tools. |
...
- src/data/item_packs/cotton_wool.ts
...
- Add or improve only `cotton_wool`.
...
- Whether `cotton_wool` was added, improved, merged into an existing id or rejected.
