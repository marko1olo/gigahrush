# Items 097: decon_completion_stamp

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `decon_completion_stamp` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:89 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Cleanup completion proof |
| reachability target | HQ/Maintenance |
| implementation note | Pairs with cleanup gear. |
...
- src/data/item_packs/decon_completion_stamp.ts
...
- Add or improve only `decon_completion_stamp`.
...
- Whether `decon_completion_stamp` was added, improved, merged into an existing id or rejected.
