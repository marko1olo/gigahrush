# Items 077: napalm_mix

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `napalm_mix` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:127 (Liquidator weapons, ammo and combat roles)
...
| mode | new |
| intended type | AMMO |
| gameplay role | ROKS/SHMK fuel |
| reachability target | Factory/HQ |
| implementation note | Map to fuel; maybe alias to ammo_fuel behavior. |
...
- src/data/item_packs/napalm_mix.ts plus src/data/weapon_packs/napalm_mix.ts for weapons
...
- Add or improve only `napalm_mix`.
...
- Whether `napalm_mix` was added, improved, merged into an existing id or rejected.
