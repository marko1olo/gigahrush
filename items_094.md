# Items 094: nii_sample_label

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `nii_sample_label` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:86 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Sample official seal |
...
- src/data/item_packs/nii_sample_label.ts
...
- Add or improve only `nii_sample_label`.
...
- Whether `nii_sample_label` was added, improved, merged into an existing id or rejected.
