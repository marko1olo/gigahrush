# Items 126: slime_age_label_brown

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `slime_age_label_brown` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:82 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `slime_age_label_brown` |
| Russian name | Бирка молодой слизи |
| mode | new |
| intended type | MISC |
| gameplay role | Sample classification token |
| reachability target | NII/cleanup |
| implementation note | Could be combined with document item. |
...
- src/data/item_packs/slime_age_label_brown.ts
...
- Add or improve only `slime_age_label_brown`.
...
- Whether `slime_age_label_brown` was added, improved, merged into an existing id or rejected.
