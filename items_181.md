# Items 181: rail_signal_lamp

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `rail_signal_lamp` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:94 (Trade, production, contraband, electronics and resident goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Rail/terminal repair |
| reachability target | Depot/office |
| implementation note | Electronics resource. |
...
- src/data/item_packs/rail_signal_lamp.ts
...
- Add or improve only `rail_signal_lamp`.
...
- Whether `rail_signal_lamp` was added, improved, merged into an existing id or rejected.
