# Items 095: contaminated_sample_act

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `contaminated_sample_act` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:87 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Failure/evidence branch |
| reachability target | NII/aftermath |
| implementation note | Generated by failed sample handling. |
...
- src/data/item_packs/contaminated_sample_act.ts
...
- Add or improve only `contaminated_sample_act`.
...
- Whether `contaminated_sample_act` was added, improved, merged into an existing id or rejected.
