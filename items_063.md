# Items 063: o15_multijet_flamer

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `o15_multijet_flamer` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:113 (Liquidator weapons, ammo and combat roles)
...
| Field | Value |
| --- | --- |
| id | `o15_multijet_flamer` |
| Russian name | 6О15-УТТХ |
| mode | new |
| intended type | WEAPON |
| gameplay role | Engineer breach flamethrower |
| reachability target | Deep engineer stash |
| implementation note | Defer if too dominant. |
...
- src/data/item_packs/o15_multijet_flamer.ts plus src/data/weapon_packs/o15_multijet_flamer.ts for weapons
...
- Add or improve only `o15_multijet_flamer`.
...
- Whether `o15_multijet_flamer` was added, improved, merged into an existing id or rejected.
