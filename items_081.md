# Items 081: liquidator_issue_card

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `liquidator_issue_card` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:73 (Documents, permits, bureaucracy and access goods)
...
| Field | Value |
| --- | --- |
| id | `liquidator_issue_card` |
| Russian name | Карточка выдачи ликвидатора |
| mode | new |
| intended type | MISC |
| gameplay role | Legal access to one field kit |
| reachability target | HQ/Ministry |
| implementation note | Consumed or stamped by stash interaction. |
...
- src/data/item_packs/liquidator_issue_card.ts
...
- Add or improve only `liquidator_issue_card`.
...
- Whether `liquidator_issue_card` was added, improved, merged into an existing id or rejected.
