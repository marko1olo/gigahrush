# Items 038: homemade_pistol

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `homemade_pistol` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:88 (Liquidator weapons, ammo and combat roles)
...
| gameplay role | Existing craft pistol |
| reachability target | Current item |
| implementation note | Add blueprint/ammo source through items_5. |
...
- src/data/item_packs/homemade_pistol.ts plus src/data/weapon_packs/homemade_pistol.ts for weapons
...
- Add or improve only `homemade_pistol`.
...
- Whether `homemade_pistol` was added, improved, merged into an existing id or rejected.
