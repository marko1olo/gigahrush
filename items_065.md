# Items 065: brt2_foam_projector

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `brt2_foam_projector` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:115 (Liquidator weapons, ammo and combat roles)
...
| Field | Value |
| --- | --- |
| id | `brt2_foam_projector` |
| Russian name | БРТ-2 бетономёт |
| mode | new |
| intended type | WEAPON |
| gameplay role | Foam terrain/immobilizer |
| reachability target | Engineer route |
| implementation note | Needs generic temporary-block/slow effect. |
...
- src/data/item_packs/brt2_foam_projector.ts plus src/data/weapon_packs/brt2_foam_projector.ts for weapons
...
- Add or improve only `brt2_foam_projector`.
...
- Whether `brt2_foam_projector` was added, improved, merged into an existing id or rejected.
