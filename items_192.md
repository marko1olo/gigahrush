# Items 192: rubber_tube

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `rubber_tube` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:105 (Trade, production, contraband, electronics and resident goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Still/medical/repair input |
| reachability target | Medical/production |
| implementation note | Useful recipe piece. |
...
- src/data/item_packs/rubber_tube.ts
...
- Add or improve only `rubber_tube`.
...
- Whether `rubber_tube` was added, improved, merged into an existing id or rejected.
