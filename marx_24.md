...
## Текущая задача
**Арена: Мастер Арены. Диалоги, интерфейс выбора боев и ставок.**



### Контекст задачи
НПЦ «Мастер Арены» — ключевой персонаж для интерфейса ставок и выбора боёв.

### Конкретные файлы и паттерны
- **`src/data/plot.ts`**: Зарегистрируйте НПЦ через `registerSideQuest()` или напрямую через `registerNpcPackageFromPlotNpc()`. `plotNpcId: 'arena_master'`, faction: `Faction.LIQUIDATOR`, occupation: `Occupation.DIRECTOR`.
- **`src/systems/interactions.ts`**: NPC main-menu rows строятся через conditional option registry. Добавьте опцию `'Арена'` для `arena_master`, открывающую canvas overlay.
- **`src/render/`**: Создайте новый UI файл `arena_ui.ts` для оверлея выбора боёв (по паттерну `gambling_ui.ts` или `computer_ui.ts`).
- **Меню**: Две кнопки: «Ставки на бой» (зрительский режим) и «Выйти на арену» (боевой режим игрока).
