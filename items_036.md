# Items 036: shock_baton

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `shock_baton` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:86 (Liquidator weapons, ammo and combat roles)
...
| gameplay role | Human control, low monster damage |
| reachability target | OVB/HQ/black market |
| implementation note | Stun only through generic status if available. |
...
- src/data/item_packs/shock_baton.ts plus src/data/weapon_packs/shock_baton.ts for weapons
...
- Add or improve only `shock_baton`.
...
- Whether `shock_baton` was added, improved, merged into an existing id or rejected.
