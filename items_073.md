# Items 073: ammo_12g_slug

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `ammo_12g_slug` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:123 (Liquidator weapons, ammo and combat roles)
...
| mode | new |
| intended type | AMMO |
| gameplay role | Precise shotgun ammo |
| reachability target | Factory/HQ |
| implementation note | Requires ammo-selection or weapon-specific entry. |
...
- src/data/item_packs/ammo_12g_slug.ts plus src/data/weapon_packs/ammo_12g_slug.ts for weapons
...
- Add or improve only `ammo_12g_slug`.
...
- Whether `ammo_12g_slug` was added, improved, merged into an existing id or rejected.
