# Items 189: ceramic_shards_pack

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `ceramic_shards_pack` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:102 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `ceramic_shards_pack` |
| Russian name | Керамика |
| mode | new |
| intended type | MISC |
| gameplay role | Craft/insulation material |
| reachability target | Vending/storage |
| implementation note | Source automagazine. |
...
- src/data/item_packs/ceramic_shards_pack.ts
...
- Add or improve only `ceramic_shards_pack`.
...
- Whether `ceramic_shards_pack` was added, improved, merged into an existing id or rejected.
