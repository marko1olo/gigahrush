# Items 187: aerosol_paint_maiden

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `aerosol_paint_maiden` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:100 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `aerosol_paint_maiden` |
| Russian name | Аэрозольная краска «цвет девства» |
| mode | new |
| intended type | MISC |
| gameplay role | Tag/mark/contraband item |
| reachability target | Storage/black market |
| implementation note | Avoid visible render feature unless generic. |
...
- src/data/item_packs/aerosol_paint_maiden.ts
...
- Add or improve only `aerosol_paint_maiden`.
...
- Whether `aerosol_paint_maiden` was added, improved, merged into an existing id or rejected.
