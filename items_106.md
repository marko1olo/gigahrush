# Items 106: blueprint_t2_folder

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `blueprint_t2_folder` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:98 (Documents, permits, bureaucracy and access goods)
- items_5.md:71 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `blueprint_t2_folder` |
| Russian name | Папка чертежей Т2 |
| mode | new |
| intended type | MISC |
| gameplay role | Better recipe unlock |
| reachability target | Fibrous capsules/terminal |
...
- items_3.md:98: Better recipe unlock; Coordinate with items_5.
- items_5.md:71: Better recipe unlock; Use factory recipe gates later.
...
- src/data/item_packs/blueprint_t2_folder.ts
...
- Add or improve only `blueprint_t2_folder`.
...
- Whether `blueprint_t2_folder` was added, improved, merged into an existing id or rejected.
