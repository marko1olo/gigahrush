# Items 186: roller_brush

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `roller_brush` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:99 (Trade, production, contraband, electronics and resident goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Painting/repair item |
| reachability target | Vending/storage |
| implementation note | Could support wall marks/HARMS later. |
...
- src/data/item_packs/roller_brush.ts
...
- Add or improve only `roller_brush`.
...
- Whether `roller_brush` was added, improved, merged into an existing id or rejected.
