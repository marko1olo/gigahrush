...
### Контекст задачи
Текстуры стен на высоких потолках дублируются (особенно двери и надписи повторяются вертикально).

### Конкретные файлы и паттерны
- **`src/render/webgl.ts`**: В raycaster при отрисовке стены UV-координата `v` масштабируется по высоте стены. Если стена высокая — текстура тайлится.
- **Решение 1**: Для текстур `Tex.DOOR_WOOD`, `Tex.DOOR_METAL`, `Tex.DOOR_HERMETIC` — рисовать дверь только в нижней половине, а верхнюю часть заполнить стандартной текстурой стены (overlay).
- **Решение 2**: Для текстур с надписями (`Tex.POSTER_BASE`, `Tex.SCREEN_BASE`) — ограничить их вертикальный extent.
- **`src/render/textures.ts`**: Процедурные текстуры. Можно модифицировать генерацию, чтобы верхняя часть 64x64 тайла была нейтральной.
