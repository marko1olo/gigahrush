# Items 166: black_market_shells

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `black_market_shells` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:79 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `black_market_shells` |
| Russian name | Чёрнорыночная дробь |
| mode | new |
| intended type | AMMO |
| gameplay role | Illegal shell source |
| reachability target | Black market |
| implementation note | Must resource-map and be reachable. |
...
- src/data/item_packs/black_market_shells.ts
...
- Add or improve only `black_market_shells`.
...
- Whether `black_market_shells` was added, improved, merged into an existing id or rejected.
