# Items 010: rubber_door_wedge

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `rubber_door_wedge` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:78 (Liquidator cleanup, PPE and samosbor field gear)
...
| Field | Value |
| --- | --- |
| id | `rubber_door_wedge` |
| Russian name | Резиновый клин гермодвери |
| mode | new |
| intended type | MISC |
| gameplay role | Door hold / quick seal proof |
| reachability target | Maintenance, storage |
| implementation note | Use only through generic door/interaction hooks. |
...
- src/data/item_packs/rubber_door_wedge.ts
...
- Add or improve only `rubber_door_wedge`.
...
- Whether `rubber_door_wedge` was added, improved, merged into an existing id or rejected.
