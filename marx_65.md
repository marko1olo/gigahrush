...
### Контекст задачи
Добавить экологию гномов: где обитают, контр-плей, что дропают.

### Конкретные файлы и паттерны
- **`src/data/monster_ecology.ts`**: Добавьте запись `GNOME`:
  ```ts
  { kind: MonsterKind.GNOME, floors: [FloorLevel.MAINTENANCE, FloorLevel.LIVING], roomPreference: [RoomType.STORAGE, RoomType.CORRIDOR], counterplay: '...', lootHint: '...', rumorIds: ['gnome_sighting'] }
  ```
- **`src/data/rumors.ts`**: Добавьте 2-3 слуха про гномов: «В коллекторах видели мелких. Шустрые, гады.»
- **Спавн**: В генераторах Maintenance и Living — добавьте `MonsterKind.GNOME` в monster mix с весом ~5% от общего пула.
