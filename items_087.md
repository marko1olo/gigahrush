# Items 087: fuel_issue_stamp

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `fuel_issue_stamp` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:79 (Documents, permits, bureaucracy and access goods)
...
| --- | --- |
| id | `fuel_issue_stamp` |
| Russian name | Штамп выдачи топлива |
| mode | new |
| intended type | MISC |
| gameplay role | ROKS/SHMK fuel bureaucracy |
| reachability target | Production/HQ |
| implementation note | Links to ammo_fuel/napalm_mix. |
...
- src/data/item_packs/fuel_issue_stamp.ts
...
- Add or improve only `fuel_issue_stamp`.
...
- Whether `fuel_issue_stamp` was added, improved, merged into an existing id or rejected.
