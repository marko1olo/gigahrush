# Items 060: roks47_flamethrower

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `roks47_flamethrower` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:110 (Liquidator weapons, ammo and combat roles)
...
| Field | Value |
| --- | --- |
| id | `roks47_flamethrower` |
| Russian name | РОКС-47 |
| mode | new |
| intended type | WEAPON |
| gameplay role | Signature backpack napalm |
| reachability target | Liquidator permit/stash |
| implementation note | Stronger fuel economy, heavier value. |
...
- src/data/item_packs/roks47_flamethrower.ts plus src/data/weapon_packs/roks47_flamethrower.ts for weapons
...
- Add or improve only `roks47_flamethrower`.
...
- Whether `roks47_flamethrower` was added, improved, merged into an existing id or rejected.
