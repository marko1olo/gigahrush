...
### Контекст задачи
Дать мобам (гулям, мутантам) шанс нести технический или биологический лут.

### Конкретные файлы и паттерны
- **`src/entities/monster.ts`**: Текущие монстры имеют `lootHint` но не реальный inventory.
- **`src/systems/procedural_loot.ts`**: Добавьте функцию `generateMonsterLoot(kind: MonsterKind): Item[]` — она возвращает 0-3 предмета на основе ecology data.
- **`src/data/monster_ecology.ts`**: Добавьте поле `lootTable?: { defId: string, chance: number }[]` к каждой ecology entry.
- **Death**: В обработчике смерти монстра — дропать сгенерированный лут.
