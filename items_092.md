# Items 092: contraband_receipt_blank

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `contraband_receipt_blank` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:84 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Forgery input |
| reachability target | Smoking/office |
| implementation note | Pairs with black market. |
...
- src/data/item_packs/contraband_receipt_blank.ts
...
- Add or improve only `contraband_receipt_blank`.
...
- Whether `contraband_receipt_blank` was added, improved, merged into an existing id or rejected.
