# Items 193: bottle_empty

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `bottle_empty` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:106 (Trade, production, contraband, electronics and resident goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Water/reagent/brewing container |
| reachability target | Kitchens/living |
...
- src/data/item_packs/bottle_empty.ts
...
- Add or improve only `bottle_empty`.
...
- Whether `bottle_empty` was added, improved, merged into an existing id or rejected.
