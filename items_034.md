# Items 034: rake_bayonet

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `rake_bayonet` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:84 (Liquidator weapons, ammo and combat roles)
...
| gameplay role | Weird rifle fallback |
| reachability target | Rare liquidator stash |
| implementation note | Implement as standalone melee unless attachment system exists. |
...
- src/data/item_packs/rake_bayonet.ts plus src/data/weapon_packs/rake_bayonet.ts for weapons
...
- Add or improve only `rake_bayonet`.
...
- Whether `rake_bayonet` was added, improved, merged into an existing id or rejected.
