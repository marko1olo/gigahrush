# Items 190: cardboard_stack

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `cardboard_stack` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:103 (Trade, production, contraband, electronics and resident goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Craft/barter material |
| reachability target | Vending/living |
| implementation note | Cheap resource. |
...
- src/data/item_packs/cardboard_stack.ts
...
- Add or improve only `cardboard_stack`.
...
- Whether `cardboard_stack` was added, improved, merged into an existing id or rejected.
