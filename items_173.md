# Items 173: heating_element

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `heating_element` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:86 (Trade, production, contraband, electronics and resident goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Still/heater/thaw input |
| reachability target | Production/kitchen |
...
- src/data/item_packs/heating_element.ts
...
- Add or improve only `heating_element`.
...
- Whether `heating_element` was added, improved, merged into an existing id or rejected.
