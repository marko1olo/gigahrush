# Items 120: cracked_sample_jar

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `cracked_sample_jar` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:76 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `cracked_sample_jar` |
| Russian name | Треснувшая банка для пробы |
| mode | new |
| intended type | MISC |
| gameplay role | Bad sample container |
| reachability target | Storage/aftermath |
| implementation note | Leaks/low value. |
...
- src/data/item_packs/cracked_sample_jar.ts
...
- Add or improve only `cracked_sample_jar`.
...
- Whether `cracked_sample_jar` was added, improved, merged into an existing id or rejected.
