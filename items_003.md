# Items 003: uv_spotlight

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `uv_spotlight` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:71 (Liquidator cleanup, PPE and samosbor field gear)
...
| mode | improve |
| intended type | TOOL |
| gameplay role | Counterplay against eye/skinless/black traces |
| reachability target | HQ, liquidator stashes |
| implementation note | Add tags liquidator, cleanup, uv. |
...
- src/data/item_packs/uv_spotlight.ts
...
- Add or improve only `uv_spotlight`.
...
- Whether `uv_spotlight` was added, improved, merged into an existing id or rejected.
