...
## Текущая задача
**База Ликвидаторов: Регистрация в manifest.ts и design_floors.ts.**



### Контекст задачи
Финальная регистрация Базы Ликвидаторов в маршрутной системе.

### Конкретные файлы и паттерны
- **`src/data/design_floors.ts`**: Убедитесь, что запись `liquidator_base` полная: `id`, `hudName`, `baseFloor`, `z`, `themeClass`.
- **`src/gen/design_floors/manifest.ts`**: Import + `registerDesignFloor('liquidator_base', generate...)`.
- **`src/systems/procedural_floors.ts`**: Убедитесь, что route deck не конфликтует с выбранным `z`.
- **Debug**: Добавьте debug teleport в `src/systems/debug.ts`: `'/tp liquidator_base'`.
- **Smoke test**: Запустите `npm run check` — генератор должен не падать.
