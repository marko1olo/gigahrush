# Items 088: gusl_index_page

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `gusl_index_page` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:80 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | NOTE |
| gameplay role | Lore/access clue |
| reachability target | Office/HQ/archive |
| implementation note | Keep Russian display; id uses gusl. |
...
- src/data/item_packs/gusl_index_page.ts
...
- Add or improve only `gusl_index_page`.
...
- Whether `gusl_index_page` was added, improved, merged into an existing id or rejected.
