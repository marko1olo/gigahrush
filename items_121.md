# Items 121: sterile_swab

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `sterile_swab` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:77 (NII sampleware, slime, medicine, food and anomaly loot)
...
| mode | new |
| intended type | MISC |
| gameplay role | Small evidence sample |
| reachability target | Medical/NII |
| implementation note | Stackable sample input. |
...
- src/data/item_packs/sterile_swab.ts
...
- Add or improve only `sterile_swab`.
...
- Whether `sterile_swab` was added, improved, merged into an existing id or rejected.
