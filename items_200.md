# Items 200: stolen_terminal_stamp

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `stolen_terminal_stamp` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:113 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `stolen_terminal_stamp` |
| Russian name | Украденная печать терминала |
| mode | new |
| intended type | MISC |
| gameplay role | Forgery/economy risk |
| reachability target | Office/black market |
| implementation note | Coordinate with items_3. |
...
- src/data/item_packs/stolen_terminal_stamp.ts
...
- Add or improve only `stolen_terminal_stamp`.
...
- Whether `stolen_terminal_stamp` was added, improved, merged into an existing id or rejected.
