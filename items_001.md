# Items 001: liquidator_rake

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `liquidator_rake` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:69 (Liquidator cleanup, PPE and samosbor field gear)
- items_2.md:81 (Liquidator weapons, ammo and combat roles)
...
| --- | --- |
| id | `liquidator_rake` |
| Russian name | Грабли ликвидатора 0Г15 |
| mode | new |
| intended type | WEAPON |
| gameplay role | Long weak melee plus slime cleanup identity |
| reachability target | Liquidator stashes, samosbor cleanup contracts |
| implementation note | Stats in items_2 only if it becomes combat-relevant. |
...
- items_1.md:69: Long weak melee plus slime cleanup identity; Stats in items_2 only if it becomes combat-relevant.
- items_2.md:81: Iconic cleanup reach melee; Keep weaker than `fire_hook`, add slime-clean tags.
...
- src/data/item_packs/liquidator_rake.ts
- src/data/item_packs/liquidator_rake.ts plus src/data/weapon_packs/liquidator_rake.ts for weapons
...
- Add or improve only `liquidator_rake`.
...
- Whether `liquidator_rake` was added, improved, merged into an existing id or rejected.
