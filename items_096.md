# Items 096: quarantine_breach_notice

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `quarantine_breach_notice` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:88 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Consequence document |
| reachability target | Medical/Ministry |
| implementation note | Event output or quest item. |
...
- src/data/item_packs/quarantine_breach_notice.ts
...
- Add or improve only `quarantine_breach_notice`.
...
- Whether `quarantine_breach_notice` was added, improved, merged into an existing id or rejected.
