# Items 099: passport_stub

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `passport_stub` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:91 (Documents, permits, bureaucracy and access goods)
...
| mode | improve |
| intended type | MISC |
| gameplay role | Existing identity paper |
| reachability target | Current item |
| implementation note | Use instead of new if already present. |
...
- src/data/item_packs/passport_stub.ts
...
- Add or improve only `passport_stub`.
...
- Whether `passport_stub` was added, improved, merged into an existing id or rejected.
