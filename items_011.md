# Items 011: hermo_gasket

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `hermo_gasket` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:79 (Liquidator cleanup, PPE and samosbor field gear)
...
| mode | improve |
| intended type | MISC |
| gameplay role | Door/lift seal repair |
| reachability target | Maintenance, storage |
| implementation note | Pair with sealant_tube, asbestos_cord. |
...
- src/data/item_packs/hermo_gasket.ts
...
- Add or improve only `hermo_gasket`.
...
- Whether `hermo_gasket` was added, improved, merged into an existing id or rejected.
