# Items 022: radio_headset_liquidator

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `radio_headset_liquidator` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:90 (Liquidator cleanup, PPE and samosbor field gear)
...
| Field | Value |
| --- | --- |
| id | `radio_headset_liquidator` |
| Russian name | Гарнитура ликвидатора |
| mode | new |
| intended type | TOOL |
| gameplay role | Heard-radius/radio affordance |
| reachability target | HQ, trade |
| implementation note | Could modify message radius through existing context later. |
...
- src/data/item_packs/radio_headset_liquidator.ts
...
- Add or improve only `radio_headset_liquidator`.
...
- If active behavior is needed, use a generic inventory use handler and bounded durability/cooldown.
- Do not put gameplay state in render code.
...
- Whether `radio_headset_liquidator` was added, improved, merged into an existing id or rejected.
