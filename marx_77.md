...
### Контекст задачи
Живность (мухи/крысы/тараканы) должна появляться изредка и быть привязана к «грязным» клеткам.

### Конкретные файлы и паттерны
- **`src/render/critters.ts`** (от marx_74): Не спавнить в чистых, хорошо освещённых комнатах. Привязка:
  - Крысы: ячейки рядом с контейнерами, `RoomType.STORAGE`, `RoomType.KITCHEN`.
  - Тараканы: `RoomType.BATHROOM`, тёмные коридоры (low light value).
  - Мухи: только над трупами и органическими marks.
- **`world.light`**: Typed array освещения. Critters предпочитают dark cells (light < 30).
- **Частота**: Максимум 30 critters на экран в радиусе 15.
