...
### Контекст задачи
НПЦ должны умнее управлять инвентарём: подбирать лучшее оружие, выбрасывать мусор.

### Конкретные файлы и паттерны
- **`src/systems/ai/npc_utility.ts`**: Текущая система микро-целей. Добавьте intent `'loot_upgrade'`: если НПЦ видит item drop с weapon value > текущего оружия — идти подобрать.
- **`src/systems/inventory.ts`**: Функции `pickupItem()` / `dropItem()` уже есть. НПЦ должны оценивать: `newWeaponDmg > currentWeaponDmg` → подобрать, выбросить старое.
- **Broadphase**: Используйте `systems/entity_index.ts` для поиска ближайших item drops, НЕ full-array scan.
- **Cooldown**: intent `loot_upgrade` — низкий приоритет, cadence 30 секунд.
