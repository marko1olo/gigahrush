# Items 161: homemade_ammo_instruction

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `homemade_ammo_instruction` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:74 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `homemade_ammo_instruction` |
| Russian name | Инструкция кустарных патронов |
| mode | new |
| intended type | MISC |
| gameplay role | Enables homemade ammo path |
| reachability target | Cabinets/black market |
| implementation note | Source-backed from homemade pistol page. |
...
- src/data/item_packs/homemade_ammo_instruction.ts
...
- Add or improve only `homemade_ammo_instruction`.
...
- Whether `homemade_ammo_instruction` was added, improved, merged into an existing id or rejected.
