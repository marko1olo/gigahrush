...
### Контекст задачи
Добавить дверям параметр «прочность» (HP), чтобы они могли быть разрушены.
...
- **`src/systems/interactions.ts`**: В обработчике удара по двери (`E` action или melee hit) — уменьшайте `door.hp`. Когда `hp <= 0` → `door.state = DoorState.OPEN` + bump `world.cellVersion` (для перестройки nav tree).
