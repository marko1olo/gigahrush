# Items 078: empty_roks_tank

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `empty_roks_tank` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:128 (Liquidator weapons, ammo and combat roles)
...
| --- | --- |
| id | `empty_roks_tank` |
| Russian name | Пустой ранцевый бак |
| mode | new |
| intended type | MISC |
| gameplay role | Flamethrower repair/refill |
| reachability target | HQ/factory |
| implementation note | Production input. |
...
- src/data/item_packs/empty_roks_tank.ts plus src/data/weapon_packs/empty_roks_tank.ts for weapons
...
- Add or improve only `empty_roks_tank`.
...
- Whether `empty_roks_tank` was added, improved, merged into an existing id or rejected.
