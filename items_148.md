# Items 148: grey_briquette

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `grey_briquette` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:104 (NII sampleware, slime, medicine, food and anomaly loot)
...
| mode | improve |
| intended type | FOOD |
| gameplay role | Existing common concentrate |
| reachability target | Current item |
| implementation note | Align with source general concentrate. |
...
- src/data/item_packs/grey_briquette.ts
...
- Add or improve only `grey_briquette`.
...
- Whether `grey_briquette` was added, improved, merged into an existing id or rejected.
