# Items 138: sleeping_pills

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `sleeping_pills` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:94 (NII sampleware, slime, medicine, food and anomaly loot)
...
| gameplay role | Forced rest / risk |
| reachability target | Medical/black market |
| implementation note | Defer if sleep action not generic. |
...
- src/data/item_packs/sleeping_pills.ts
...
- Add or improve only `sleeping_pills`.
...
- Whether `sleeping_pills` was added, improved, merged into an existing id or rejected.
