# Items 035: liquidator_axe

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `liquidator_axe` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:85 (Liquidator weapons, ammo and combat roles)
...
| --- | --- |
| id | `liquidator_axe` |
| Russian name | Топор ликвидатора |
| mode | new |
| intended type | WEAPON |
| gameplay role | Heavy door/body/slime work |
| reachability target | HQ/cleanup rooms |
| implementation note | Sidegrade to axe, more durability, slower. |
...
- src/data/item_packs/liquidator_axe.ts plus src/data/weapon_packs/liquidator_axe.ts for weapons
...
- Add or improve only `liquidator_axe`.
...
- Whether `liquidator_axe` was added, improved, merged into an existing id or rejected.
