# Items 067: pbrog1_foam_launcher

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `pbrog1_foam_launcher` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:117 (Liquidator weapons, ammo and combat roles)
...
| Field | Value |
| --- | --- |
| id | `pbrog1_foam_launcher` |
| Russian name | ПБРОГ-1 |
| mode | new |
| intended type | WEAPON |
| gameplay role | One-shot ranged foam |
| reachability target | Rare engineer crate |
| implementation note | Model as launcher, not ammo subtype first. |
...
- src/data/item_packs/pbrog1_foam_launcher.ts plus src/data/weapon_packs/pbrog1_foam_launcher.ts for weapons
...
- Add or improve only `pbrog1_foam_launcher`.
...
- Whether `pbrog1_foam_launcher` was added, improved, merged into an existing id or rejected.
