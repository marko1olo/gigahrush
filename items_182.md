# Items 182: rail_spike_pack

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `rail_spike_pack` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:95 (Trade, production, contraband, electronics and resident goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Rail repair material |
| reachability target | Depot/production |
| implementation note | Metal resource. |
...
- src/data/item_packs/rail_spike_pack.ts
...
- Add or improve only `rail_spike_pack`.
...
- Whether `rail_spike_pack` was added, improved, merged into an existing id or rejected.
