# Items 014: alkali_powder

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `alkali_powder` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:82 (Liquidator cleanup, PPE and samosbor field gear)
...
| mode | new |
| intended type | MISC |
| gameplay role | Brown slime cleanup input |
| reachability target | Storage/production |
| implementation note | Pairs with brown_slime_cleanup_act. |
...
- src/data/item_packs/alkali_powder.ts
...
- Add or improve only `alkali_powder`.
...
- Whether `alkali_powder` was added, improved, merged into an existing id or rejected.
