# Items 108: rail_depot_pass

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `rail_depot_pass` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:100 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Railway/transport route access |
| reachability target | Transport offices |
| implementation note | Source: rail docs are missing/extracted. |
...
- src/data/item_packs/rail_depot_pass.ts
...
- Add or improve only `rail_depot_pass`.
...
- Whether `rail_depot_pass` was added, improved, merged into an existing id or rejected.
