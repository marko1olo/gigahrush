# Items 066: foam_grenade_6p10

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `foam_grenade_6p10` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:116 (Liquidator weapons, ammo and combat roles)
...
| Field | Value |
| --- | --- |
| id | `foam_grenade_6p10` |
| Russian name | Пенобетонная граната 6П10 |
| mode | new |
| intended type | WEAPON |
| gameplay role | Throwable control |
| reachability target | Liquidator crates |
| implementation note | If no foam geometry, make slow/stun grenade. |
...
- src/data/item_packs/foam_grenade_6p10.ts plus src/data/weapon_packs/foam_grenade_6p10.ts for weapons
...
- Add or improve only `foam_grenade_6p10`.
...
- Whether `foam_grenade_6p10` was added, improved, merged into an existing id or rejected.
