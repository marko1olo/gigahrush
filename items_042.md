# Items 042: eralashnikov_auto

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `eralashnikov_auto` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:92 (Liquidator weapons, ammo and combat roles)
...
| Field | Value |
| --- | --- |
| id | `eralashnikov_auto` |
| Russian name | Автомат Ералашникова |
| mode | new |
| intended type | WEAPON |
| gameplay role | Main liquidator automatic rifle |
| reachability target | Permit/HQ/black market |
| implementation note | Use ammo_762 or new rifle ammo only if resource mapped. |
...
- src/data/item_packs/eralashnikov_auto.ts plus src/data/weapon_packs/eralashnikov_auto.ts for weapons
...
- Add or improve only `eralashnikov_auto`.
...
- Whether `eralashnikov_auto` was added, improved, merged into an existing id or rejected.
