# Items 021: portable_siren_key

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `portable_siren_key` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:89 (Liquidator cleanup, PPE and samosbor field gear)
...
| Field | Value |
| --- | --- |
| id | `portable_siren_key` |
| Russian name | Ключ переносной сирены |
| mode | new |
| intended type | MISC |
| gameplay role | Warning system repair token |
| reachability target | HQ/office |
| implementation note | Do not add new siren runtime unless generic. |
...
- src/data/item_packs/portable_siren_key.ts
...
- Add or improve only `portable_siren_key`.
...
- Whether `portable_siren_key` was added, improved, merged into an existing id or rejected.
