...
// Build body HTML starting with a REGULAR paragraph (NOT a quote)
let html = '';

// FIRST LINE: Regular paragraph text!
...
// SECOND LINE: Project channel quote
...
(function() {
  ...
  if (bodyEl) {
    ...
    // Remove any leftover red placeholder errors
    const errors = document.querySelectorAll('.m_error, .m_empty');
    errors.forEach(e => e.classList.remove('m_error', 'm_empty'));

    bodyEl.dispatchEvent(new Event('input', { bubbles: true }));
    bodyEl.dispatchEvent(new Event('change', { bubbles: true }));
  }
  return 'FIRST_LINE_PARAGRAPH_FIXED';
})();
`;

fs.writeFileSync('/tmp/fix_first_line.js', jsCode);

const applescriptCode = `
set jsCode to read POSIX file "/tmp/fix_first_line.js"
tell application "Google Chrome"
  execute tab 16 of window 1 javascript jsCode
end tell
`;

fs.writeFileSync('/tmp/fix_first_line.scpt', applescriptCode);
const res = execSync('osascript /tmp/fix_first_line.scpt').toString();
