...
(function() {
  ...
  if (bodyEl) {
    ...
    // Native execCommand insertHTML (updates ProseMirror state tree!)
    const ok = document.execCommand('insertHTML', false, html);
    
    bodyEl.dispatchEvent(new Event('input', { bubbles: true }));
    bodyEl.dispatchEvent(new Event('change', { bubbles: true }));

    return 'EXEC_COMMAND_INSERT_HTML_' + ok;
  }
  return 'NO_BODY';
})();
`;

fs.writeFileSync('/tmp/insert_html.js', jsCode);

const applescriptCode = `
set jsCode to read POSIX file "/tmp/insert_html.js" as «class utf8»
tell application "Google Chrome"
  execute tab 16 of window 1 javascript jsCode
end tell
`;

fs.writeFileSync('/tmp/insert_html.scpt', applescriptCode);
const res = execSync('osascript /tmp/insert_html.scpt').toString();
