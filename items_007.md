# Items 007: used_gasmask_filter

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `used_gasmask_filter` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:75 (Liquidator cleanup, PPE and samosbor field gear)
...
| Field | Value |
| --- | --- |
| id | `used_gasmask_filter` |
| Russian name | Отработанный фильтр |
| mode | new |
| intended type | MISC |
| gameplay role | Evidence/junk after PPE use |
| reachability target | Produced by use handler |
| implementation note | Sell cheap, audit contamination. |
...
- src/data/item_packs/used_gasmask_filter.ts
...
- Add or improve only `used_gasmask_filter`.
...
- Whether `used_gasmask_filter` was added, improved, merged into an existing id or rejected.
