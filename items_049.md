# Items 049: chizh3_shotgun

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `chizh3_shotgun` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:99 (Liquidator weapons, ammo and combat roles)
...
| gameplay role | Official pump shotgun |
| reachability target | Liquidator permit/stash |
| implementation note | Treat as shotgun source variant per items.md. |
...
- src/data/item_packs/chizh3_shotgun.ts plus src/data/weapon_packs/chizh3_shotgun.ts for weapons
...
- Add or improve only `chizh3_shotgun`.
...
- Whether `chizh3_shotgun` was added, improved, merged into an existing id or rejected.
