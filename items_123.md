# Items 123: sample_cork_seal

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `sample_cork_seal` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:79 (NII sampleware, slime, medicine, food and anomaly loot)
...
| mode | new |
| intended type | MISC |
| gameplay role | Sealing consumable |
| reachability target | Office/medical |
| implementation note | Pairs with sample jar. |
...
- src/data/item_packs/sample_cork_seal.ts
...
- Add or improve only `sample_cork_seal`.
...
- Whether `sample_cork_seal` was added, improved, merged into an existing id or rejected.
