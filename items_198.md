# Items 198: resident_trinket_box

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `resident_trinket_box` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:111 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `resident_trinket_box` |
| Russian name | Коробка жильцовых мелочей |
| mode | new |
| intended type | MISC |
| gameplay role | Loot table wrapper/trade good |
| reachability target | Living/storage |
| implementation note | Data-only value item. |
...
- src/data/item_packs/resident_trinket_box.ts
...
- Add or improve only `resident_trinket_box`.
...
- Whether `resident_trinket_box` was added, improved, merged into an existing id or rejected.
