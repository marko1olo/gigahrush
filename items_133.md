# Items 133: boiled_slime_residue

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `boiled_slime_residue` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:89 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `boiled_slime_residue` |
| Russian name | Вываренный остаток слизи |
| mode | new |
| intended type | MISC |
| gameplay role | Heat counterplay proof |
| reachability target | NII/cleanup |
| implementation note | Could be output of decon/heat. |
...
- src/data/item_packs/boiled_slime_residue.ts
...
- Add or improve only `boiled_slime_residue`.
...
- Whether `boiled_slime_residue` was added, improved, merged into an existing id or rejected.
