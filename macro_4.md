# macro_4: Content audit script improvement
...
Improve `npm run content:audit` so content-shipping mistakes are actionable.
...
Add one to three narrow checks for manifest imports, undefined zone titles, item references, or unreachable content; keep the script Node-only and avoid duplicating all data-id tests.
