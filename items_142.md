# Items 142: syringe_empty

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `syringe_empty` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:98 (NII sampleware, slime, medicine, food and anomaly loot)
...
| mode | new |
| intended type | MISC |
| gameplay role | Injection component |
| reachability target | Medical/NII |
| implementation note | Use with serum/poison later. |
...
- src/data/item_packs/syringe_empty.ts
...
- Add or improve only `syringe_empty`.
...
- Whether `syringe_empty` was added, improved, merged into an existing id or rejected.
