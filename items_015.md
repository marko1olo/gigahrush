# Items 015: lime_bucket

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `lime_bucket` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:83 (Liquidator cleanup, PPE and samosbor field gear)
...
| --- | --- |
| id | `lime_bucket` |
| Russian name | Ведро извести |
| mode | new |
| intended type | MISC |
| gameplay role | Body/slime sanitation token |
| reachability target | Production/storage |
| implementation note | Heavy trade/evidence item. |
...
- src/data/item_packs/lime_bucket.ts
...
- Add or improve only `lime_bucket`.
...
- Whether `lime_bucket` was added, improved, merged into an existing id or rejected.
