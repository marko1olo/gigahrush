# Items 100: part_ticket

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `part_ticket` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:92 (Documents, permits, bureaucracy and access goods)
...
| --- | --- |
| id | `part_ticket` |
| Russian name | Партбилет |
| mode | new |
| intended type | MISC |
| gameplay role | High-status terminal/work access |
| reachability target | Office/Ministry |
| implementation note | Source adaptation; use cautiously. |
...
- src/data/item_packs/part_ticket.ts
...
- Add or improve only `part_ticket`.
...
- Whether `part_ticket` was added, improved, merged into an existing id or rejected.
