# Items 111: hermodoor_journal

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `hermodoor_journal` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:103 (Documents, permits, bureaucracy and access goods)
...
| Field | Value |
| --- | --- |
| id | `hermodoor_journal` |
| Russian name | Журнал гермодвери |
| mode | improve |
| intended type | MISC |
| gameplay role | Existing door document |
| reachability target | Current item |
| implementation note | Add tags or use path if weak. |
...
- src/data/item_packs/hermodoor_journal.ts
...
- Add or improve only `hermodoor_journal`.
...
- Whether `hermodoor_journal` was added, improved, merged into an existing id or rejected.
