# Items 158: fibrous_capsule_cut

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `fibrous_capsule_cut` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:114 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `fibrous_capsule_cut` |
| Russian name | Срез фиброзной капсулы |
| mode | new |
| intended type | MISC |
| gameplay role | Meat-block hazard loot |
| reachability target | Samosbor aftermath |
| implementation note | Good source for blueprints/items_5. |
...
- src/data/item_packs/fibrous_capsule_cut.ts
...
- Add or improve only `fibrous_capsule_cut`.
...
- Whether `fibrous_capsule_cut` was added, improved, merged into an existing id or rejected.
