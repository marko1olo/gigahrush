# Items 131: slime_sense_node

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `slime_sense_node` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:87 (NII sampleware, slime, medicine, food and anomaly loot)
...
| --- | --- |
| id | `slime_sense_node` |
| Russian name | Чувствительный узел слизи |
| mode | new |
| intended type | MISC |
| gameplay role | Echolocation/odor sample |
| reachability target | Rare slime aftermath |
| implementation note | Could link to detectors later. |
...
- src/data/item_packs/slime_sense_node.ts
...
- Add or improve only `slime_sense_node`.
...
- Whether `slime_sense_node` was added, improved, merged into an existing id or rejected.
