# Items 064: ato41_atomic_flamer

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `ato41_atomic_flamer` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:114 (Liquidator weapons, ammo and combat roles)
...
| Field | Value |
| --- | --- |
| id | `ato41_atomic_flamer` |
| Russian name | АТО-41 |
| mode | new |
| intended type | WEAPON |
| gameplay role | Endgame door/slime cutter |
| reachability target | Unique route |
| implementation note | Needs strict cap and collateral. |
...
- src/data/item_packs/ato41_atomic_flamer.ts plus src/data/weapon_packs/ato41_atomic_flamer.ts for weapons
...
- Add or improve only `ato41_atomic_flamer`.
...
- Whether `ato41_atomic_flamer` was added, improved, merged into an existing id or rejected.
