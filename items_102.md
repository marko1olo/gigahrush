# Items 102: hazard_shift_extension

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `hazard_shift_extension` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:94 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Dangerous production work permit |
| reachability target | Factory/HQ |
| implementation note | Risk/reward document. |
...
- src/data/item_packs/hazard_shift_extension.ts
...
- Add or improve only `hazard_shift_extension`.
...
- Whether `hazard_shift_extension` was added, improved, merged into an existing id or rejected.
