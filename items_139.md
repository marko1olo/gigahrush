# Items 139: antiemetic

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `antiemetic` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:95 (NII sampleware, slime, medicine, food and anomaly loot)
...
| gameplay role | Nausea/food preservation |
| reachability target | Medical |
| implementation note | Useful only with nausea/poison status. |
...
- src/data/item_packs/antiemetic.ts
...
- Add or improve only `antiemetic`.
...
- Whether `antiemetic` was added, improved, merged into an existing id or rejected.
