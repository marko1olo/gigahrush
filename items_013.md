# Items 013: decon_fluid

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `decon_fluid` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:81 (Liquidator cleanup, PPE and samosbor field gear)
...
| mode | new |
| intended type | MISC |
| gameplay role | Slime/fungus cleanup reagent |
| reachability target | Medical, Maintenance, liquidator stash |
| implementation note | Use handler can publish cleanup event. |
...
- src/data/item_packs/decon_fluid.ts
...
- Add or improve only `decon_fluid`.
...
- Whether `decon_fluid` was added, improved, merged into an existing id or rejected.
