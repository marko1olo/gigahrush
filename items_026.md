# Items 026: protective_apron

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `protective_apron` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:94 (Liquidator cleanup, PPE and samosbor field gear)
...
| mode | new |
| intended type | MISC |
| gameplay role | Lab/cleanup clothing token |
| reachability target | NII/medical |
| implementation note | Trade/evidence until protection hooks exist. |
...
- src/data/item_packs/protective_apron.ts
...
- Add or improve only `protective_apron`.
...
- Whether `protective_apron` was added, improved, merged into an existing id or rejected.
