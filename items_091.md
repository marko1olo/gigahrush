# Items 091: confiscation_tag

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `confiscation_tag` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:83 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Converts illegal item into relation/evidence choice |
| reachability target | Ministry/HQ |
| implementation note | Existing document market hooks likely enough. |
...
- src/data/item_packs/confiscation_tag.ts
...
- Add or improve only `confiscation_tag`.
...
- Whether `confiscation_tag` was added, improved, merged into an existing id or rejected.
