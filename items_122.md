# Items 122: contaminated_swab

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `contaminated_swab` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:78 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `contaminated_swab` |
| Russian name | Загрязнённый мазок |
| mode | new |
| intended type | MISC |
| gameplay role | Failed handling output |
| reachability target | Use handler/event |
| implementation note | Low-value evidence. |
...
- src/data/item_packs/contaminated_swab.ts
...
- Add or improve only `contaminated_swab`.
...
- Whether `contaminated_swab` was added, improved, merged into an existing id or rejected.
