# Items 005: p14_gasmask_receipt

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `p14_gasmask_receipt` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:73 (Liquidator cleanup, PPE and samosbor field gear)
...
| Field | Value |
| --- | --- |
| id | `p14_gasmask_receipt` |
| Russian name | Квитанция 8П14 |
| mode | new |
| intended type | MISC |
| gameplay role | Paper access to gas mask issue |
| reachability target | Office/HQ |
| implementation note | Pairs with filter_receipt. |
...
- src/data/item_packs/p14_gasmask_receipt.ts
...
- Add or improve only `p14_gasmask_receipt`.
...
- Whether `p14_gasmask_receipt` was added, improved, merged into an existing id or rejected.
