# Items 090: foam_grenade_act

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `foam_grenade_act` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:82 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Foam grenade legal path |
| reachability target | HQ |
| implementation note | Pairs with items_2. |
...
- src/data/item_packs/foam_grenade_act.ts
...
- Add or improve only `foam_grenade_act`.
...
- Whether `foam_grenade_act` was added, improved, merged into an existing id or rejected.
