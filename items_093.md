# Items 093: sample_chain_form

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `sample_chain_form` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:85 (Documents, permits, bureaucracy and access goods)
...
| Field | Value |
| --- | --- |
| id | `sample_chain_form` |
| Russian name | Бланк цепочки пробы |
| mode | new |
| intended type | MISC |
| gameplay role | Legalizes sample handoff |
| reachability target | NII/office |
| implementation note | Coordinate with items_4. |
...
- src/data/item_packs/sample_chain_form.ts
...
- Add or improve only `sample_chain_form`.
...
- Whether `sample_chain_form` was added, improved, merged into an existing id or rejected.
