# Items 039: karkarov_pistol

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `karkarov_pistol` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:89 (Liquidator weapons, ammo and combat roles)
...
| gameplay role | Official weak service pistol |
| reachability target | Liquidator trade/permit |
| implementation note | Sidegrade to makarov, less value or different spread. |
...
- src/data/item_packs/karkarov_pistol.ts plus src/data/weapon_packs/karkarov_pistol.ts for weapons
...
- Add or improve only `karkarov_pistol`.
...
- Whether `karkarov_pistol` was added, improved, merged into an existing id or rejected.
