# Items 156: water_reservoir_sample

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `water_reservoir_sample` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:112 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `water_reservoir_sample` |
| Russian name | Проба воды из резервуара |
| mode | new |
| intended type | MISC |
| gameplay role | Water safety evidence |
| reachability target | Bathrooms/kitchens |
| implementation note | Works with water system/economy. |
...
- src/data/item_packs/water_reservoir_sample.ts
...
- Add or improve only `water_reservoir_sample`.
...
- Whether `water_reservoir_sample` was added, improved, merged into an existing id or rejected.
