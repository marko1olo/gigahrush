# Items 076: ammo_rifle_coupon

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `ammo_rifle_coupon` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:126 (Liquidator weapons, ammo and combat roles)
...
| mode | new |
| intended type | MISC |
| gameplay role | Ammo issue gating |
| reachability target | HQ/office |
| implementation note | May be better than many new ammo ids. |
...
- src/data/item_packs/ammo_rifle_coupon.ts plus src/data/weapon_packs/ammo_rifle_coupon.ts for weapons
...
- Add or improve only `ammo_rifle_coupon`.
...
- Whether `ammo_rifle_coupon` was added, improved, merged into an existing id or rejected.
