# Items 059: flamethrower

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `flamethrower` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:109 (Liquidator weapons, ammo and combat roles)
...
| gameplay role | Existing fire clear |
| reachability target | Current item |
| implementation note | Keep as generic/industrial baseline. |
...
- src/data/item_packs/flamethrower.ts plus src/data/weapon_packs/flamethrower.ts for weapons
...
- Add or improve only `flamethrower`.
...
- Whether `flamethrower` was added, improved, merged into an existing id or rejected.
