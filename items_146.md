...
Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `soap_72` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:102 (NII sampleware, slime, medicine, food and anomaly loot)
...
| --- | --- |
| id | `soap_72` |
| Russian name | Мыло хозяйственное 72% |
| mode | new |
| intended type | MISC |
| gameplay role | Hygiene/decontamination |
| reachability target | Bathrooms/vending |
| implementation note | Could reduce contamination/parasite later. |
...
- src/data/item_packs/soap_72.ts
...
- Whether `soap_72` was added, improved, merged into an existing id or rejected.
