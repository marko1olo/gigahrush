# Items 085: ammo_coupon_9mm

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `ammo_coupon_9mm` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:77 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Legal pistol ammo access |
| reachability target | Office/HQ |
| implementation note | Prefer token over generic ammo spawn. |
...
- src/data/item_packs/ammo_coupon_9mm.ts
...
- Add or improve only `ammo_coupon_9mm`.
...
- Whether `ammo_coupon_9mm` was added, improved, merged into an existing id or rejected.
