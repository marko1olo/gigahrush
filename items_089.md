# Items 089: gusl_index_fragment

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `gusl_index_fragment` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:81 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Classifies weird weapon |
| reachability target | Loot/office |
| implementation note | Trade or unlock hint. |
...
- src/data/item_packs/gusl_index_fragment.ts
...
- Add or improve only `gusl_index_fragment`.
...
- Whether `gusl_index_fragment` was added, improved, merged into an existing id or rejected.
