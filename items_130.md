# Items 130: slime_motor_node

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `slime_motor_node` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:86 (NII sampleware, slime, medicine, food and anomaly loot)
...
| mode | new |
| intended type | MISC |
| gameplay role | Weird research sample |
| reachability target | Rare slime aftermath |
| implementation note | Lore from primitive motor. |
...
- src/data/item_packs/slime_motor_node.ts
...
- Add or improve only `slime_motor_node`.
...
- Whether `slime_motor_node` was added, improved, merged into an existing id or rejected.
