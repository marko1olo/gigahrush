# Items 125: gas_sample_ampoule

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `gas_sample_ampoule` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:81 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `gas_sample_ampoule` |
| Russian name | Ампула газовой пробы |
| mode | new |
| intended type | MISC |
| gameplay role | NII gas evidence |
| reachability target | Gas aftermath/NII |
| implementation note | No global gas sim required. |
...
- src/data/item_packs/gas_sample_ampoule.ts
...
- Add or improve only `gas_sample_ampoule`.
...
- Whether `gas_sample_ampoule` was added, improved, merged into an existing id or rejected.
