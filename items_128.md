# Items 128: slime_age_label_violet

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `slime_age_label_violet` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:84 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `slime_age_label_violet` |
| Russian name | Бирка взрослой слизи |
| mode | new |
| intended type | MISC |
| gameplay role | High-tier dangerous proof |
| reachability target | NII/cleanup |
| implementation note | Keep rare. |
...
- src/data/item_packs/slime_age_label_violet.ts
...
- Add or improve only `slime_age_label_violet`.
...
- Whether `slime_age_label_violet` was added, improved, merged into an existing id or rejected.
