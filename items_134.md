# Items 134: antifungal_ointment

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `antifungal_ointment` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:90 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `antifungal_ointment` |
| Russian name | Противогрибковая мазь |
| mode | improve |
| intended type | MEDICINE |
| gameplay role | Existing fungal medicine |
| reachability target | Current item |
| implementation note | Keep as core fungus counter. |
...
- src/data/item_packs/antifungal_ointment.ts
...
- Add or improve only `antifungal_ointment`.
...
- Whether `antifungal_ointment` was added, improved, merged into an existing id or rejected.
