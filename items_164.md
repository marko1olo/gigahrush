# Items 164: scrubbed_serial_plate

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `scrubbed_serial_plate` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:77 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `scrubbed_serial_plate` |
| Russian name | Сбитая номерная планка |
| mode | new |
| intended type | MISC |
| gameplay role | Contraband weapon proof |
| reachability target | Black market |
| implementation note | Audit/trade decision. |
...
- src/data/item_packs/scrubbed_serial_plate.ts
...
- Add or improve only `scrubbed_serial_plate`.
...
- Whether `scrubbed_serial_plate` was added, improved, merged into an existing id or rejected.
