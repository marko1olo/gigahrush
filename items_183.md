# Items 183: track_diagram_scrap

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `track_diagram_scrap` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:96 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `track_diagram_scrap` |
| Russian name | Обрывок схемы путей |
| mode | new |
| intended type | MISC |
| gameplay role | Transport clue |
| reachability target | Depot/office |
| implementation note | Source says docs were removed. |
...
- src/data/item_packs/track_diagram_scrap.ts
...
- Add or improve only `track_diagram_scrap`.
...
- Whether `track_diagram_scrap` was added, improved, merged into an existing id or rejected.
