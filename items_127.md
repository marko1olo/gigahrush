# Items 127: slime_age_label_orange

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `slime_age_label_orange` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:83 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `slime_age_label_orange` |
| Russian name | Бирка подростковой слизи |
| mode | new |
| intended type | MISC |
| gameplay role | More dangerous sample proof |
| reachability target | NII/cleanup |
| implementation note | Source: age/color slime. |
...
- src/data/item_packs/slime_age_label_orange.ts
...
- Add or improve only `slime_age_label_orange`.
...
- Whether `slime_age_label_orange` was added, improved, merged into an existing id or rejected.
