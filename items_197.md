...
Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `dice_bone` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:110 (Trade, production, contraband, electronics and resident goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Gambling/social good |
| reachability target | Living/common |
| implementation note | Connect to existing gambling machines. |
...
- src/data/item_packs/dice_bone.ts
...
- Whether `dice_bone` was added, improved, merged into an existing id or rejected.
