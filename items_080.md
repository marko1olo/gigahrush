# Items 080: rifle_bolt_pack

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `rifle_bolt_pack` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:130 (Liquidator weapons, ammo and combat roles)
...
| mode | new |
| intended type | AMMO |
| gameplay role | Лосяш/rail rifle ammo |
| reachability target | Deep route/factory |
| implementation note | Could reuse ammo_harpoon if redundant. |
...
- src/data/item_packs/rifle_bolt_pack.ts plus src/data/weapon_packs/rifle_bolt_pack.ts for weapons
...
- Add or improve only `rifle_bolt_pack`.
...
- Whether `rifle_bolt_pack` was added, improved, merged into an existing id or rejected.
