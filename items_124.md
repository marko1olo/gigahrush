# Items 124: glass_ampoule_empty

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `glass_ampoule_empty` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:80 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `glass_ampoule_empty` |
| Russian name | Пустая ампула |
| mode | new |
| intended type | MISC |
| gameplay role | Medicine/sample input |
| reachability target | Medical/NII |
| implementation note | Generic component. |
...
- src/data/item_packs/glass_ampoule_empty.ts
...
- Add or improve only `glass_ampoule_empty`.
...
- Whether `glass_ampoule_empty` was added, improved, merged into an existing id or rejected.
