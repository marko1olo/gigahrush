# Items 180: rail_switch_handle

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `rail_switch_handle` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:93 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `rail_switch_handle` |
| Russian name | Рукоять стрелочного перевода |
| mode | new |
| intended type | MISC |
| gameplay role | Railway route repair |
| reachability target | Depot/transport |
| implementation note | Coordinate with route content. |
...
- src/data/item_packs/rail_switch_handle.ts
...
- Add or improve only `rail_switch_handle`.
...
- Whether `rail_switch_handle` was added, improved, merged into an existing id or rejected.
