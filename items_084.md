# Items 084: scrubbed_weapon_tag

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `scrubbed_weapon_tag` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:76 (Documents, permits, bureaucracy and access goods)
...
| Russian name | Сбитая оружейная бирка |
| mode | new |
| intended type | MISC |
| gameplay role | Contraband marker |
| reachability target | Black market |
| implementation note | Use in trade/audit choices. |
...
- src/data/item_packs/scrubbed_weapon_tag.ts
...
- Add or improve only `scrubbed_weapon_tag`.
...
- Whether `scrubbed_weapon_tag` was added, improved, merged into an existing id or rejected.
