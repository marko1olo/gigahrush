# Items 069: concrete_breaker_grenade

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `concrete_breaker_grenade` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:119 (Liquidator weapons, ammo and combat roles)
...
| Field | Value |
| --- | --- |
| id | `concrete_breaker_grenade` |
| Russian name | Бетонобойная граната |
| mode | new |
| intended type | WEAPON |
| gameplay role | Breach grenade |
| reachability target | Engineer stash |
| implementation note | Avoid full-world damage scans. |
...
- src/data/item_packs/concrete_breaker_grenade.ts plus src/data/weapon_packs/concrete_breaker_grenade.ts for weapons
...
- Add or improve only `concrete_breaker_grenade`.
...
- Whether `concrete_breaker_grenade` was added, improved, merged into an existing id or rejected.
