# Items 185: toiletpaper

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `toiletpaper` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:98 (Trade, production, contraband, electronics and resident goods)
...
| --- | --- |
| id | `toiletpaper` |
| Russian name | Туалетная бумага |
| mode | improve |
| intended type | MISC |
| gameplay role | Existing barter/hygiene |
| reachability target | Current item |
| implementation note | Add vending/trade role rather than duplicate if no difference. |
...
- src/data/item_packs/toiletpaper.ts
...
- Add or improve only `toiletpaper`.
...
- Whether `toiletpaper` was added, improved, merged into an existing id or rejected.
