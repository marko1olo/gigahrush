# Items 196: moonshine_still_part

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `moonshine_still_part` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:109 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `moonshine_still_part` |
| Russian name | Деталь самогонного аппарата |
| mode | new |
| intended type | MISC |
| gameplay role | Contraband production input |
| reachability target | Production/black market |
| implementation note | No full brewing UI unless assigned. |
...
- src/data/item_packs/moonshine_still_part.ts
...
- Add or improve only `moonshine_still_part`.
...
- Whether `moonshine_still_part` was added, improved, merged into an existing id or rejected.
