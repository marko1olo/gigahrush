# Items 162: barrel_part

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `barrel_part` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:75 (Trade, production, contraband, electronics and resident goods)
...
- src/data/item_packs/barrel_part.ts
...
- Add or improve only `barrel_part`.
...
- Whether `barrel_part` was added, improved, merged into an existing id or rejected.
