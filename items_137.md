# Items 137: painkiller_pack

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `painkiller_pack` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:93 (NII sampleware, slime, medicine, food and anomaly loot)
...
| --- | --- |
| id | `painkiller_pack` |
| Russian name | Болеутоляющее |
| mode | new |
| intended type | MEDICINE |
| gameplay role | Aim/move penalty relief |
| reachability target | Medical/vending |
| implementation note | If no pain status, use small HP + sleep tradeoff. |
...
- src/data/item_packs/painkiller_pack.ts
...
- Add or improve only `painkiller_pack`.
...
- Whether `painkiller_pack` was added, improved, merged into an existing id or rejected.
