# Items 169: sound_emitter

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `sound_emitter` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:82 (Trade, production, contraband, electronics and resident goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Craft/input or lure |
| reachability target | Cabinets |
| implementation note | Existing noise_can may cover active lure. |
...
- src/data/item_packs/sound_emitter.ts
...
- Add or improve only `sound_emitter`.
...
- Whether `sound_emitter` was added, improved, merged into an existing id or rejected.
