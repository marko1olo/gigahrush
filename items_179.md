# Items 179: vent_damper_plate

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `vent_damper_plate` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:92 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `vent_damper_plate` |
| Russian name | Заслонка вентиляции |
| mode | new |
| intended type | MISC |
| gameplay role | Vent repair/control |
| reachability target | Maintenance |
| implementation note | No global vent sim. |
...
- src/data/item_packs/vent_damper_plate.ts
...
- Add or improve only `vent_damper_plate`.
...
- Whether `vent_damper_plate` was added, improved, merged into an existing id or rejected.
