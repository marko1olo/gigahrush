# Items 199: party_portrait_pin

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `party_portrait_pin` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:112 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `party_portrait_pin` |
| Russian name | Значок с портрета партии |
| mode | new |
| intended type | MISC |
| gameplay role | Bureaucratic bribe/junk |
| reachability target | Office/common |
| implementation note | Low-value flavor with trade use. |
...
- src/data/item_packs/party_portrait_pin.ts
...
- Add or improve only `party_portrait_pin`.
...
- Whether `party_portrait_pin` was added, improved, merged into an existing id or rejected.
