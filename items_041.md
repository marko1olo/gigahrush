# Items 041: slyoznev_pps41

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `slyoznev_pps41` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:91 (Liquidator weapons, ammo and combat roles)
...
| --- | --- |
| id | `slyoznev_pps41` |
| Russian name | ППС-41 Слизнёва |
| mode | new |
| intended type | WEAPON |
| gameplay role | Early SMG, fast ammo burn |
| reachability target | Liquidator recruit stash |
| implementation note | Sidegrade to ppsh; no heat unless generic. |
...
- src/data/item_packs/slyoznev_pps41.ts plus src/data/weapon_packs/slyoznev_pps41.ts for weapons
...
- Add or improve only `slyoznev_pps41`.
...
- Whether `slyoznev_pps41` was added, improved, merged into an existing id or rejected.
