# Items 032: contaminated_gloves

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `contaminated_gloves` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:100 (Liquidator cleanup, PPE and samosbor field gear)
...
| Field | Value |
| --- | --- |
| id | `contaminated_gloves` |
| Russian name | Загрязнённые перчатки |
| mode | new |
| intended type | MISC |
| gameplay role | Bad cleanup consequence |
| reachability target | Generated by failed handling |
| implementation note | Low-value evidence/contraband. |
...
- src/data/item_packs/contaminated_gloves.ts
...
- Add or improve only `contaminated_gloves`.
...
- Whether `contaminated_gloves` was added, improved, merged into an existing id or rejected.
