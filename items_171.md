# Items 171: screen_unit

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `screen_unit` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:84 (Trade, production, contraband, electronics and resident goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Terminal repair part |
...
- src/data/item_packs/screen_unit.ts
...
- Add or improve only `screen_unit`.
...
- Whether `screen_unit` was added, improved, merged into an existing id or rejected.
