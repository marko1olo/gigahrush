...
Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `bayonet` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:83 (Liquidator weapons, ammo and combat roles)
...
| gameplay role | Existing reach melee |
| reachability target | Current item |
| implementation note | Could become rifle attachment fiction without attachment system. |
...
- src/data/item_packs/bayonet.ts plus src/data/weapon_packs/bayonet.ts for weapons
...
- Whether `bayonet` was added, improved, merged into an existing id or rejected.
