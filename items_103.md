# Items 103: terminal_order_receipt

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `terminal_order_receipt` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:95 (Documents, permits, bureaucracy and access goods)
...
| Russian name | Квитанция терминального заказа |
| mode | new |
| intended type | MISC |
| gameplay role | Delayed mail/craft proof |
| reachability target | Terminals/office |
| implementation note | Good for future delivery system. |
...
- src/data/item_packs/terminal_order_receipt.ts
...
- Add or improve only `terminal_order_receipt`.
...
- Whether `terminal_order_receipt` was added, improved, merged into an existing id or rejected.
