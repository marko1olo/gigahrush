...
### Контекст задачи
Лампы должны автоматически подстраиваться под высоту потолка комнаты, в которой они стоят.

### Конкретные файлы и паттерны
- **`src/render/webgl.ts`**: Найдите, где рисуются светильники (`Feature.LAMP`). Их Y-координата (высота) должна зависеть от `Room.ceilingTier` текущей комнаты.
- **`src/core/world.ts`**: `world.roomMap[idx]` → получаем room id → `world.rooms[roomId].ceilingTier`.
- **Формула**: `lampHeight = baseCeilingHeight - offset`, где `baseCeilingHeight` рассчитывается из `ceilingTier`.
- **Тестирование**: Визуально проверьте на dev-сервере (`npm run dev`) на этажах Коллекторов и Министерства.
