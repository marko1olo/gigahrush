# Items 045: nosin_rifle

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `nosin_rifle` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:95 (Liquidator weapons, ammo and combat roles)
...
| gameplay role | Militia bolt rifle |
| reachability target | Opolchenets/civilian stash |
| implementation note | Slow, cheap ammo. |
...
- src/data/item_packs/nosin_rifle.ts plus src/data/weapon_packs/nosin_rifle.ts for weapons
...
- Add or improve only `nosin_rifle`.
...
- Whether `nosin_rifle` was added, improved, merged into an existing id or rejected.
