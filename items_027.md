# Items 027: cleanup_order_stub

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `cleanup_order_stub` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:95 (Liquidator cleanup, PPE and samosbor field gear)
...
| Field | Value |
| --- | --- |
| id | `cleanup_order_stub` |
| Russian name | Корешок приказа на зачистку |
| mode | new |
| intended type | MISC |
| gameplay role | Legal proof for loot/confiscation |
| reachability target | Office/HQ |
| implementation note | Goes to items_3 if document-heavy. |
...
- src/data/item_packs/cleanup_order_stub.ts
...
- Add or improve only `cleanup_order_stub`.
...
- Whether `cleanup_order_stub` was added, improved, merged into an existing id or rejected.
