# Items 083: weapon_checkout_tag

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `weapon_checkout_tag` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:75 (Documents, permits, bureaucracy and access goods)
...
| Field | Value |
| --- | --- |
| id | `weapon_checkout_tag` |
| Russian name | Оружейная бирка |
| mode | new |
| intended type | MISC |
| gameplay role | Audit proof for issued weapon |
| reachability target | Armory/HQ |
| implementation note | Pair with stolen weapons. |
...
- src/data/item_packs/weapon_checkout_tag.ts
...
- Add or improve only `weapon_checkout_tag`.
...
- Whether `weapon_checkout_tag` was added, improved, merged into an existing id or rejected.
