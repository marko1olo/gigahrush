...
Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `ozk_patch` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:93 (Liquidator cleanup, PPE and samosbor field gear)
...
| --- | --- |
| id | `ozk_patch` |
| Russian name | Заплата ОЗК |
| mode | new |
| intended type | MISC |
| gameplay role | PPE repair input |
| reachability target | Medical/HQ/storage |
| implementation note | No armor system unless planned. |
...
- src/data/item_packs/ozk_patch.ts
...
- Whether `ozk_patch` was added, improved, merged into an existing id or rejected.
