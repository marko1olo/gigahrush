# Items 009: asbestos_cord

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `asbestos_cord` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:77 (Liquidator cleanup, PPE and samosbor field gear)
...
| mode | improve |
| intended type | MISC |
| gameplay role | Hermetic sealing input |
| reachability target | Production/storage |
| implementation note | Treat as existing item, add tags/reachability if needed. |
...
- src/data/item_packs/asbestos_cord.ts
...
- Add or improve only `asbestos_cord`.
...
- Whether `asbestos_cord` was added, improved, merged into an existing id or rejected.
