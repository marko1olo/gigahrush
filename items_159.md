# Items 159: frozen_item_shard

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `frozen_item_shard` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:115 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `frozen_item_shard` |
| Russian name | Осколок замороженного предмета |
| mode | new |
| intended type | MISC |
| gameplay role | Ice-block loot residue |
| reachability target | Cold/deep route |
| implementation note | Unlocks rare recipes. |
...
- src/data/item_packs/frozen_item_shard.ts
...
- Add or improve only `frozen_item_shard`.
...
- Whether `frozen_item_shard` was added, improved, merged into an existing id or rejected.
