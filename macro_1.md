# macro_1: P0 QA gate: content audit blockers
...
Make the current landed content pass audit before new work builds on unstable data.
...
Run `npm run content:audit`, fix the missing `rubles` reference through the existing money/item model, add stable metadata for `samosbornyy_ostov`, and keep README factual only after verified counts change.
