# Items 201: market_weight_scale

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `market_weight_scale` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:114 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `market_weight_scale` |
| Russian name | Рыночные весы |
| mode | new |
| intended type | MISC |
| gameplay role | Trade stall proof/item |
| reachability target | Market/storage |
| implementation note | Future economy content. |
...
- src/data/item_packs/market_weight_scale.ts
...
- Add or improve only `market_weight_scale`.
...
- Whether `market_weight_scale` was added, improved, merged into an existing id or rejected.
