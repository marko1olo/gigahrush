# Items 135: anti_spore_inhaler

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `anti_spore_inhaler` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:91 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `anti_spore_inhaler` |
| Russian name | Противоспоровый ингалятор |
| mode | new |
| intended type | MEDICINE |
| gameplay role | Respiratory medicine |
| reachability target | Medical/NII |
| implementation note | One-use PSI/HP or status relief. |
...
- src/data/item_packs/anti_spore_inhaler.ts
...
- Add or improve only `anti_spore_inhaler`.
...
- Whether `anti_spore_inhaler` was added, improved, merged into an existing id or rejected.
