# Items 070: chest_failsafe_charge

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `chest_failsafe_charge` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:120 (Liquidator weapons, ammo and combat roles)
...
| Field | Value |
| --- | --- |
| id | `chest_failsafe_charge` |
| Russian name | Фугасный нагрудный заряд |
| mode | new |
| intended type | WEAPON |
| gameplay role | Last-resort consequence item |
| reachability target | Story/rare |
| implementation note | Likely defer for quest content. |
...
- src/data/item_packs/chest_failsafe_charge.ts plus src/data/weapon_packs/chest_failsafe_charge.ts for weapons
...
- Add or improve only `chest_failsafe_charge`.
...
- Whether `chest_failsafe_charge` was added, improved, merged into an existing id or rejected.
