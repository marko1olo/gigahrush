...
### Контекст задачи
Лимитировать количество одновременно активных групп фракций (из marx_62) для производительности.

### Конкретные файлы и паттерны
- **`src/data/entity_limits.ts`**: Здесь лимиты (4096 NPC+monster actors). Добавьте аналогичный лимит для macro goals: max 3 assault groups.
- **`src/systems/factions.ts`**: Перед созданием нового `FactionMacroGoal` — проверять лимит.
- **GC**: Завершённые/неактуальные macro goals — удалять из массива.
- **Cadence**: Оценка macro goals — не чаще 1 раз в 60 секунд реального времени.
