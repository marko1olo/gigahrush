# Items 109: rail_switch_order

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `rail_switch_order` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:101 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Route reroute token |
| reachability target | Depot/office |
| implementation note | Future floor route hook, no new enum. |
...
- src/data/item_packs/rail_switch_order.ts
...
- Add or improve only `rail_switch_order`.
...
- Whether `rail_switch_order` was added, improved, merged into an existing id or rejected.
