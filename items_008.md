# Items 008: wet_rag_bundle

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `wet_rag_bundle` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:76 (Liquidator cleanup, PPE and samosbor field gear)
...
| mode | new |
| intended type | MISC |
| gameplay role | Emergency gas/smoke mitigation |
| reachability target | Bathrooms/kitchens |
| implementation note | Short weak alternative, no new armor system. |
...
- src/data/item_packs/wet_rag_bundle.ts
...
- Add or improve only `wet_rag_bundle`.
...
- Whether `wet_rag_bundle` was added, improved, merged into an existing id or rejected.
