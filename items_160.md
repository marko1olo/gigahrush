# Items 160: weapon_blueprint_t2

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `weapon_blueprint_t2` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:73 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `weapon_blueprint_t2` |
| Russian name | Чертёж оружия Т2 |
| mode | new |
| intended type | MISC |
| gameplay role | Armory recipe unlock |
| reachability target | Black market/HQ |
| implementation note | Do not unlock combat directly without stats. |
...
- src/data/item_packs/weapon_blueprint_t2.ts
...
- Add or improve only `weapon_blueprint_t2`.
...
- Whether `weapon_blueprint_t2` was added, improved, merged into an existing id or rejected.
