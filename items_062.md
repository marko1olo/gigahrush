# Items 062: agnia_a130

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `agnia_a130` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:112 (Liquidator weapons, ammo and combat roles)
...
| gameplay role | Sanitary corridor flamethrower |
| reachability target | Maintenance/HQ |
| implementation note | Sidegrade to flamethrower; lower damage, better cleanup tags. |
...
- src/data/item_packs/agnia_a130.ts plus src/data/weapon_packs/agnia_a130.ts for weapons
...
- Add or improve only `agnia_a130`.
...
- Whether `agnia_a130` was added, improved, merged into an existing id or rejected.
