# Items 079: homemade_9mm

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `homemade_9mm` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:129 (Liquidator weapons, ammo and combat roles)
...
| mode | new |
| intended type | AMMO |
| gameplay role | Cheap risky pistol ammo |
| reachability target | Factory/black market |
| implementation note | Only add misfire if generic; otherwise lower value/spread via weapon. |
...
- src/data/item_packs/homemade_9mm.ts plus src/data/weapon_packs/homemade_9mm.ts for weapons
...
- Add or improve only `homemade_9mm`.
...
- Whether `homemade_9mm` was added, improved, merged into an existing id or rejected.
