# Items 047: losyash_rifle

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `losyash_rifle` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:97 (Liquidator weapons, ammo and combat roles)
...
| gameplay role | Rare anti-elite bolt/rail rifle |
| reachability target | Deep route, recon stash |
| implementation note | Could reuse harpoon projectile. |
...
- src/data/item_packs/losyash_rifle.ts plus src/data/weapon_packs/losyash_rifle.ts for weapons
...
- Add or improve only `losyash_rifle`.
...
- Whether `losyash_rifle` was added, improved, merged into an existing id or rejected.
