# Items 031: post_samosbor_probe_kit

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `post_samosbor_probe_kit` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:99 (Liquidator cleanup, PPE and samosbor field gear)
...
| Field | Value |
| --- | --- |
| id | `post_samosbor_probe_kit` |
| Russian name | Набор замера после самосбора |
| mode | new |
| intended type | MISC |
| gameplay role | Turns aftermath into NII/liquidator handoff |
| reachability target | HQ/NII |
| implementation note | High value, stack 1. |
...
- src/data/item_packs/post_samosbor_probe_kit.ts
...
- Add or improve only `post_samosbor_probe_kit`.
...
- Whether `post_samosbor_probe_kit` was added, improved, merged into an existing id or rejected.
