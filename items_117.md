# Items 117: ovb_search_warrant

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `ovb_search_warrant` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:109 (Documents, permits, bureaucracy and access goods)
...
| Russian name | Ордер ОВБ на обыск |
| mode | new |
| intended type | MISC |
| gameplay role | High-risk legal power |
| reachability target | OVB/HQ |
| implementation note | Faction/audit content later. |
...
- src/data/item_packs/ovb_search_warrant.ts
...
- Add or improve only `ovb_search_warrant`.
...
- Whether `ovb_search_warrant` was added, improved, merged into an existing id or rejected.
