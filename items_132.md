# Items 132: frozen_slime_core

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `frozen_slime_core` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:88 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `frozen_slime_core` |
| Russian name | Замороженное ядро слизи |
| mode | new |
| intended type | MISC |
| gameplay role | Freezing counterplay proof |
| reachability target | Cold route/aftermath |
| implementation note | Defer use unless cold system exists. |
...
- src/data/item_packs/frozen_slime_core.ts
...
- Add or improve only `frozen_slime_core`.
...
- Whether `frozen_slime_core` was added, improved, merged into an existing id or rejected.
