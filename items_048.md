# Items 048: tanev_svt40

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `tanev_svt40` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:98 (Liquidator weapons, ammo and combat roles)
...
| gameplay role | Endgame sniper fantasy |
| reachability target | Unique route/quest |
| implementation note | Defer if cover penetration would require red files. |
...
- src/data/item_packs/tanev_svt40.ts plus src/data/weapon_packs/tanev_svt40.ts for weapons
...
- Add or improve only `tanev_svt40`.
...
- Whether `tanev_svt40` was added, improved, merged into an existing id or rejected.
