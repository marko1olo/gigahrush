# Items 149: liquidator_ration

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `liquidator_ration` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:105 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `liquidator_ration` |
| Russian name | Черный сухпай ликвидатора |
| mode | improve |
| intended type | FOOD |
| gameplay role | Existing high nutrition |
| reachability target | Current item |
| implementation note | Align with source nutritious concentrate. |
...
- src/data/item_packs/liquidator_ration.ts
...
- Add or improve only `liquidator_ration`.
...
- Whether `liquidator_ration` was added, improved, merged into an existing id or rejected.
