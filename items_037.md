# Items 037: rubber_club

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `rubber_club` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:87 (Liquidator weapons, ammo and combat roles)
...
| gameplay role | Nonlethal-ish crowd control |
| reachability target | HQ/Ministry |
| implementation note | Low damage, high knockback. |
...
- src/data/item_packs/rubber_club.ts plus src/data/weapon_packs/rubber_club.ts for weapons
...
- Add or improve only `rubber_club`.
...
- Whether `rubber_club` was added, improved, merged into an existing id or rejected.
