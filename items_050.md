# Items 050: conscripts_doublebarrel

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `conscripts_doublebarrel` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:100 (Liquidator weapons, ammo and combat roles)
...
| Field | Value |
| --- | --- |
| id | `conscripts_doublebarrel` |
| Russian name | Двустволка срочника |
| mode | new |
| intended type | WEAPON |
| gameplay role | Low-tier two-shot shotgun |
| reachability target | Militia/civilian stash |
| implementation note | Could be toz_shotgun reframe if redundant. |
...
- src/data/item_packs/conscripts_doublebarrel.ts plus src/data/weapon_packs/conscripts_doublebarrel.ts for weapons
...
- Add or improve only `conscripts_doublebarrel`.
...
- Whether `conscripts_doublebarrel` was added, improved, merged into an existing id or rejected.
