# Items 163: magazine_part

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `magazine_part` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:76 (Trade, production, contraband, electronics and resident goods)
...
| mode | improve |
| intended type | MISC |
| gameplay role | Existing weapon component |
| reachability target | Current item |
| implementation note | Use in recipes. |
...
- src/data/item_packs/magazine_part.ts
...
- Add or improve only `magazine_part`.
...
- Whether `magazine_part` was added, improved, merged into an existing id or rejected.
