# Items 113: shelter_seat_card

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `shelter_seat_card` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:105 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Shelter access/bribe |
| reachability target | Office/common |
| implementation note | Works with shelter_tally. |
...
- src/data/item_packs/shelter_seat_card.ts
...
- Add or improve only `shelter_seat_card`.
...
- Whether `shelter_seat_card` was added, improved, merged into an existing id or rejected.
