# Items 172: krona_battery

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `krona_battery` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:85 (Trade, production, contraband, electronics and resident goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Portable power |
| reachability target | Cabinets/vending |
| implementation note | Could support tools. |
...
- src/data/item_packs/krona_battery.ts
...
- Add or improve only `krona_battery`.
...
- Whether `krona_battery` was added, improved, merged into an existing id or rejected.
