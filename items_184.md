# Items 184: import_toiletpaper

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `import_toiletpaper` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:97 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `import_toiletpaper` |
| Russian name | Туалетная бумага «Импорт» |
| mode | new |
| intended type | MISC |
| gameplay role | Vending/barter good |
| reachability target | Automagazine/bathrooms |
| implementation note | Existing toiletpaper may cover. |
...
- src/data/item_packs/import_toiletpaper.ts
...
- Add or improve only `import_toiletpaper`.
...
- Whether `import_toiletpaper` was added, improved, merged into an existing id or rejected.
