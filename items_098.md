# Items 098: resident_identity_stub

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `resident_identity_stub` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:90 (Documents, permits, bureaucracy and access goods)
...
| Russian name | Корешок удостоверения личности |
| mode | new |
| intended type | MISC |
| gameplay role | Basic identity document |
| reachability target | Office/living |
| implementation note | Existing passport_stub may cover; dedupe first. |
...
- src/data/item_packs/resident_identity_stub.ts
...
- Add or improve only `resident_identity_stub`.
...
- Whether `resident_identity_stub` was added, improved, merged into an existing id or rejected.
