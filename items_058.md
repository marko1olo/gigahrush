# Items 058: pistol_grenade_launcher

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `pistol_grenade_launcher` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:108 (Liquidator weapons, ammo and combat roles)
...
| Field | Value |
| --- | --- |
| id | `pistol_grenade_launcher` |
| Russian name | Пистолет-гранатомёт |
| mode | new |
| intended type | WEAPON |
| gameplay role | Unstable militia rare |
| reachability target | Militia stash |
| implementation note | Single shot, high self-risk. |
...
- src/data/item_packs/pistol_grenade_launcher.ts plus src/data/weapon_packs/pistol_grenade_launcher.ts for weapons
...
- Add or improve only `pistol_grenade_launcher`.
...
- Whether `pistol_grenade_launcher` was added, improved, merged into an existing id or rejected.
