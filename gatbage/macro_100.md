# macro_100: Final integration, performance, docs, and release gate
...
Merge parallel agent work, resolve conflicts, verify the game, and update only factual docs.
...
- `all changed source/test files from agent branches`
...
Collect changed file lists, preserve unrelated dirty changes, run full checks, fix real failures, summarize hot-loop/perf risks, and only update README/architecture/desdoc when verified facts or contracts changed.
...
- `npm run check`
- `npm run smoke` or `npm run check:full` when feasible
