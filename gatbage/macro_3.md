# macro_3: Data reference coherence expansion
...
Catch dead ids across quests, contracts, rumors, resources, factories, containers, faction events, lift instances, and screen signals.
...
Extend existing `node:test` assertions with precise `domain:id:field:value` failures; fix real bad data in owning files and avoid browser-only imports.
