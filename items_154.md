# Items 154: experimental_concentrate

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `experimental_concentrate` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:110 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `experimental_concentrate` |
| Russian name | Несерийный концентрат |
| mode | new |
| intended type | FOOD |
| gameplay role | Random risky ration |
| reachability target | NII/factory |
| implementation note | Avoid random effects unless bounded and tested. |
...
- src/data/item_packs/experimental_concentrate.ts
...
- Add or improve only `experimental_concentrate`.
...
- Whether `experimental_concentrate` was added, improved, merged into an existing id or rejected.
