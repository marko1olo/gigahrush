# Items 105: blueprint_t1_folder

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `blueprint_t1_folder` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:97 (Documents, permits, bureaucracy and access goods)
- items_5.md:70 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `blueprint_t1_folder` |
| Russian name | Папка чертежей Т1 |
| mode | new |
| intended type | MISC |
| gameplay role | Recipe unlock |
| reachability target | Cabinets/terminal |
...
- items_3.md:97: Recipe unlock; Coordinate with items_5.
- items_5.md:70: Basic recipe unlock; Coordinate with items_3 if document-owned.
...
- src/data/item_packs/blueprint_t1_folder.ts
...
- Add or improve only `blueprint_t1_folder`.
...
- Whether `blueprint_t1_folder` was added, improved, merged into an existing id or rejected.
