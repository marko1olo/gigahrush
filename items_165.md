# Items 165: stolen_filter_pack

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `stolen_filter_pack` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:78 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `stolen_filter_pack` |
| Russian name | Краденая пачка фильтров |
| mode | new |
| intended type | MISC |
| gameplay role | Cheap PPE supply with audit risk |
| reachability target | Black market/stolen crate |
| implementation note | Coordinate with items_1. |
...
- src/data/item_packs/stolen_filter_pack.ts
...
- Add or improve only `stolen_filter_pack`.
...
- Whether `stolen_filter_pack` was added, improved, merged into an existing id or rejected.
