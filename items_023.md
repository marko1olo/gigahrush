# Items 023: field_radio_battery

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `field_radio_battery` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:91 (Liquidator cleanup, PPE and samosbor field gear)
...
| Field | Value |
| --- | --- |
| id | `field_radio_battery` |
| Russian name | Батарея рации |
| mode | new |
| intended type | MISC |
| gameplay role | Consumable electronics pressure |
| reachability target | Storage/office |
| implementation note | Map to electronics. |
...
- src/data/item_packs/field_radio_battery.ts
...
- Add or improve only `field_radio_battery`.
...
- Whether `field_radio_battery` was added, improved, merged into an existing id or rejected.
