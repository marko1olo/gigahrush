# Items 071: grn420_gravizhernov

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `grn420_gravizhernov` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:121 (Liquidator weapons, ammo and combat roles)
...
| Field | Value |
| --- | --- |
| id | `grn420_gravizhernov` |
| Russian name | Гравижернов ГРН-420 |
| mode | new |
| intended type | WEAPON |
| gameplay role | Heavy gravity AOE |
| reachability target | Unique route/boss |
| implementation note | Existing gravity_beam_emitter may already cover role. |
...
- src/data/item_packs/grn420_gravizhernov.ts plus src/data/weapon_packs/grn420_gravizhernov.ts for weapons
...
- Add or improve only `grn420_gravizhernov`.
...
- Whether `grn420_gravizhernov` was added, improved, merged into an existing id or rejected.
