# Items 167: contraband_shocker_parts

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `contraband_shocker_parts` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:80 (Trade, production, contraband, electronics and resident goods)
...
| Field | Value |
| --- | --- |
| id | `contraband_shocker_parts` |
| Russian name | Детали шокера |
| mode | new |
| intended type | MISC |
| gameplay role | Shock baton production input |
| reachability target | Black market/electronics |
| implementation note | Coordinate with items_2. |
...
- src/data/item_packs/contraband_shocker_parts.ts
...
- Add or improve only `contraband_shocker_parts`.
...
- Whether `contraband_shocker_parts` was added, improved, merged into an existing id or rejected.
