# Items 143: permanganate_vial

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `permanganate_vial` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:99 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `permanganate_vial` |
| Russian name | Марганцовка |
| mode | new |
| intended type | MEDICINE |
| gameplay role | Poison/injection counter |
| reachability target | Medical/storage |
| implementation note | Simple medicine until poison exists. |
...
- src/data/item_packs/permanganate_vial.ts
...
- Add or improve only `permanganate_vial`.
...
- Whether `permanganate_vial` was added, improved, merged into an existing id or rejected.
