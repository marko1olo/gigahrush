# Items 020: corpse_number_tag

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `corpse_number_tag` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:88 (Liquidator cleanup, PPE and samosbor field gear)
...
| Field | Value |
| --- | --- |
| id | `corpse_number_tag` |
| Russian name | Номерок трупа |
| mode | new |
| intended type | MISC |
| gameplay role | Identity/proof token |
| reachability target | Cleanup aftermath |
| implementation note | Use with A-Life death facts only if generic. |
...
- src/data/item_packs/corpse_number_tag.ts
...
- Add or improve only `corpse_number_tag`.
...
- Whether `corpse_number_tag` was added, improved, merged into an existing id or rejected.
