...
Версия: 4.5
Дата: 2026-05-19
Статус: planning/design snapshot после project audit, markdown cleanup и QA-gate refresh; не authoritative факт-карта билда
Жанровая цель: **S.T.A.L.K.E.R.-like survival ARPG sim shooter в самосборной мегаструктуре**

`README.md` фиксирует факт текущего билда. Этот документ отвечает на другой вопрос: **что делать дальше в первую очередь, чтобы игра стала сильнее именно как survival ARPG sim shooter, а не просто больше по объему**. Фактические счетчики в секции 1 сверены через `scripts/content-audit.mjs`, точечные CLI-счеты и текущую структуру `src/`; остальные секции остаются planning/backlog, а не описанием гарантированно shipped behavior. В версии 4.5 `npm run content:audit` проходит без Errors и без unimported modules; audit pass также выявил unit-blocker в культовой процессии, закрытый фиксом минимальной группы паломников и полным повтором test gate.

Исторические агентские prompts/status/logs, batch-файлы и старые root-планы теперь лежат в `appendix.md`; исходники сохранены в `gatbage/`. Этот документ и `README.md` должны получать только сжатые важные выводы, а не новые папки логов.
...
- `src/` содержит 382 TypeScript-файла и 190 379 строк TypeScript без `dist/` и `node_modules`; без generated `src/data/bad_apple_frames.ts` это 381 файл и 147 271 строка.
- Есть 60 `node:test` файлов; `tests/net-sphere.test.ts` покрывает optional Cloudflare Net Sphere config/schema/API path, новые procedural anomaly/rail train paths получили focused tests.
...
- 299 plot NPC ids: 9 базовых и 290 side-effect registered.
- 339 side quest steps.
- 67 системных контрактов.
- 17 ресурсов экономики, 10 описаний производств, 17 production recipes, контейнеры и production ticks.
- 318 статических слухов плюс слухи из событий.
- 8 вариантов самосбора, 21 модификатор, 37 aftermath-последствий и 26 registered director beats.
- 8 номерных лифтовых инстансов.
- Вертикальный `FloorRun`: нормальный лифтовой span `z=-44..40`, 23 authored/story stops, из них 6 story `FloorLevel` anchors и 17 design floor routes/generators, плюс 62 процедурных сидовых этажа между ручными якорями без добавления новых `FloorLevel`.
- Manifest-imported content modules по аудиту: `living` 31, `ministry` 15, `kvartiry` 21, `maintenance` 45, `hell` 7, `void` 9.
- `npm run content:audit` проходит без Errors, не видит unimported content modules и подтверждает, что `samosbornyy_ostov` имеет player-facing zone title. Это не заменяет playtest: аудит доказывает связность registry/data, но не доказывает читаемость маршрута игроком.
...
| Вертикальный маршрут | `FloorRun` `z=-44..40`, 17 design floors, 62 procedural floors, numbered lift anomalies | понятная навигация: route id, danger, anomaly, return path, contract target | понимает ли игрок, куда его везет лифт и почему это не просто случайность? |
...
| Самосбор | 8 variants, 21 modifiers, 37 aftermath beats, 26 director beats, shelter/fog/seal loop | сделать warning/shelter/aftermath главной драмой каждой вылазки | переживает ли игрок самосбор решением, а не только бегом/DPS? |
...
| QA/docs | README fact map, architecture contract, content audit, 60 test files, smoke script, `gatbage/` cleanup | close audit blockers, sync counters after verified code, route audit notes | можно ли доказать shipped behavior командой и маршрутом? |
...
- **Вертикаль масштабна, но не всегда намеренна.** `z=-44..40`, 17 design floors и 62 procedural floors должны читаться как план вылазки: куда, почему, опасность, как вернуться.
