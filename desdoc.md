...
- **Макро-жизнь не равна полной realtime-симуляции невидимых этажей.** A-Life держит до `1_000_000` procedural NPC identities при достаточной памяти и `100_000` fallback; off-floor NPC сохраняют identity, floor assignment, death/override facts и могут меняться только через bounded events, migration, caravans, quests, resettlement или economy/faction consequences. Они не pathfind, не дерутся и не тикают needs вне активного этажа.
...
- `src/` содержит 510 TypeScript-файлов и 207 443 строки TypeScript без `dist/` и `node_modules`; без `src/data/bad_apple_frames.ts` это 509 файлов и 207 361 строка.
- Есть 137 `*.test.ts` `node:test` файлов; `tests/` содержит 139 файлов с учетом вспомогательных файлов. `tests/net-sphere.test.ts` покрывает optional Cloudflare Net Sphere config/schema/API path, procedural anomaly/rail train/zombie paths получили focused tests.
...
- 304 runtime plot/side NPC ids после production manifests: 9 базовых и 295 side-effect registrations.
- 349 runtime side quest steps.
...
- 578 статических слухов плюс слухи из событий.
...
- `src/data/procedural_floors.ts` - 10 geometry-профилей, 5 majority faction-профилей, 19 anomaly-профилей, danger и seed-biased loot/monster списки.
- `src/systems/procedural_floors.ts` - run seed, текущий `z`, 75 procedural/fallback specs, save/load и лифтовой маршрут.
...
2. **Weapon balance audit**: стартовое, mid-tier, late-tier оружие без дублей; учесть 32 physical weapon stat entries including unarmed fallback and deletion-beam tier.
...
Текущий `src/**/*.ts`: 510 файлов, 207 443 строки; без `src/data/bad_apple_frames.ts` это 509 файлов и 207 361 строка. Проект уже прошел рубеж 100k, поэтому 100k+ больше не цель сама по себе. Для ГИГАХРУЩА масштаб считается не строками, а количеством playable contracts, проверенных маршрутов и последствий в симуляции.
...
| Procedural anomalies | 19 anomaly profiles: teleport, smog, samosbor seed, mycelium, false safe block, Hladon cold, topology/media/crowd anomalies and normal floors | contract-aware tags, screen/rumor copy, per-anomaly counterplay | на procedural floor есть решение, отличное от "пройти коридор"? |
...
| QA/docs | README fact map, architecture contract, content audit, 137 `*.test.ts` files, smoke script, `gatbage/` cleanup | close audit blockers, sync counters after verified code, route audit notes | можно ли доказать shipped behavior командой и маршрутом? |
...
- **Вертикаль масштабна, но не всегда намеренна.** `z=-50..+50`, 20 design floors и 75 procedural/fallback floors должны читаться как план вылазки: куда, почему, опасность, как вернуться.
