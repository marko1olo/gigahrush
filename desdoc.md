...
- `src/` содержит 365 TypeScript-файлов и 169 189 строк TypeScript без `dist/` и `node_modules`; без generated `src/data/bad_apple_frames.ts` это 364 файла и 126 080 строк.
- Есть 37 `node:test` файлов; `tests/net-sphere.test.ts` покрывает optional Cloudflare Net Sphere config/schema/API path, новые procedural anomaly/rail train paths получили focused tests.
...
- Вертикальный `FloorRun`: нормальный лифтовой span `z=-44..40`, 22 authored/story stop, из них 6 story `FloorLevel` anchors и 16 design floor routes/generators, плюс 63 процедурных сидовых этажа между ручными якорями без добавления новых `FloorLevel`.
...
- **Вертикаль масштабна, но не всегда намеренна.** `z=-40..40`, 15 design floors и 60 procedural floors должны читаться как план вылазки: куда, почему, опасность, как вернуться.
