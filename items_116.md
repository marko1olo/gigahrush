# Items 116: concentrate_bonus_coupon

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `concentrate_bonus_coupon` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:108 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Better ration reward |
| reachability target | Kitchen/HQ |
| implementation note | Links to items_4 food. |
...
- src/data/item_packs/concentrate_bonus_coupon.ts
...
- Add or improve only `concentrate_bonus_coupon`.
...
- Whether `concentrate_bonus_coupon` was added, improved, merged into an existing id or rejected.
