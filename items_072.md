# Items 072: tracked_zhernov

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `tracked_zhernov` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:122 (Liquidator weapons, ammo and combat roles)
...
| --- | --- |
| id | `tracked_zhernov` |
| Russian name | Гусеничный жернов |
| mode | new |
| intended type | WEAPON |
| gameplay role | Regenerator finisher |
| reachability target | Authored route machine |
| implementation note | Better as POI interaction than inventory item. |
...
- src/data/item_packs/tracked_zhernov.ts plus src/data/weapon_packs/tracked_zhernov.ts for weapons
...
- Add or improve only `tracked_zhernov`.
...
- Whether `tracked_zhernov` was added, improved, merged into an existing id or rejected.
