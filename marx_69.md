...
### Контекст задачи
Debug оверлей для отслеживания макро-целей фракций на карте (опционально).

### Конкретные файлы и паттерны
- **`src/render/map_ui.ts`**: Полная карта. Добавьте опциональный слой «Macro Goals»: рисуйте стрелки от HQ фракции к target zone.
- **`src/systems/debug.ts`**: Добавьте debug команду `'/macro_goals'` для toggle оверлея.
- **Отображение**: Для каждой active `FactionMacroGoal` — линия от centroid зоны HQ до centroid target zone. Цвет = цвет фракции из `src/data/factions.ts`.
