# Items 191: cloth_roll

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `cloth_roll` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:104 (Trade, production, contraband, electronics and resident goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Bandage/filter/clothing input |
| reachability target | Living/storage |
| implementation note | Existing items maybe cover; dedupe. |
...
- src/data/item_packs/cloth_roll.ts
...
- Add or improve only `cloth_roll`.
...
- Whether `cloth_roll` was added, improved, merged into an existing id or rejected.
