# Items 040: zatychkin_pistol

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `zatychkin_pistol` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:90 (Liquidator weapons, ammo and combat roles)
...
| --- | --- |
| id | `zatychkin_pistol` |
| Russian name | Пистолет Затычкина |
| mode | new |
| intended type | WEAPON |
| gameplay role | Burst sidearm / officer loot |
| reachability target | OVB/officer stash |
| implementation note | Use existing projectile behavior. |
...
- src/data/item_packs/zatychkin_pistol.ts plus src/data/weapon_packs/zatychkin_pistol.ts for weapons
...
- Add or improve only `zatychkin_pistol`.
...
- Whether `zatychkin_pistol` was added, improved, merged into an existing id or rejected.
