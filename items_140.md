# Items 140: sterile_bandage

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `sterile_bandage` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:96 (NII sampleware, slime, medicine, food and anomaly loot)
...
| gameplay role | Better bleed/infection care |
| reachability target | Medical/craft |
| implementation note | If no bleed, higher heal than bandage. |
...
- src/data/item_packs/sterile_bandage.ts
...
- Add or improve only `sterile_bandage`.
...
- Whether `sterile_bandage` was added, improved, merged into an existing id or rejected.
