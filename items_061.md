# Items 061: shmk_disposable

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `shmk_disposable` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:111 (Liquidator weapons, ammo and combat roles)
...
| gameplay role | Single-use room fire panic clear |
| reachability target | Rare sealed crate |
| implementation note | Stack 1 or self-ammo; high collateral event. |
...
- src/data/item_packs/shmk_disposable.ts plus src/data/weapon_packs/shmk_disposable.ts for weapons
...
- Add or improve only `shmk_disposable`.
...
- Whether `shmk_disposable` was added, improved, merged into an existing id or rejected.
