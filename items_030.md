# Items 030: smoke_candle_check

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `smoke_candle_check` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:98 (Liquidator cleanup, PPE and samosbor field gear)
...
| Field | Value |
| --- | --- |
| id | `smoke_candle_check` |
| Russian name | Дымовая шашка проверки тяги |
| mode | new |
| intended type | MISC |
| gameplay role | Vent inspection cue |
| reachability target | Maintenance |
| implementation note | Bounded event, no gas sim. |
...
- src/data/item_packs/smoke_candle_check.ts
...
- Add or improve only `smoke_candle_check`.
...
- Whether `smoke_candle_check` was added, improved, merged into an existing id or rejected.
