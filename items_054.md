# Items 054: ptrs_liquidator

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `ptrs_liquidator` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:104 (Liquidator weapons, ammo and combat roles)
...
| gameplay role | Anti-armor/boss rifle |
| reachability target | Route or mounted encounter |
| implementation note | Defer unless heavy weapon handling is generic. |
...
- src/data/item_packs/ptrs_liquidator.ts plus src/data/weapon_packs/ptrs_liquidator.ts for weapons
...
- Add or improve only `ptrs_liquidator`.
...
- Whether `ptrs_liquidator` was added, improved, merged into an existing id or rejected.
