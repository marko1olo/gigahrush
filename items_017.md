# Items 017: nii_sample_container

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `nii_sample_container` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_1.md:85 (Liquidator cleanup, PPE and samosbor field gear)
- items_4.md:73 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `nii_sample_container` |
| Russian name | Тара НИИ для пробы |
| mode | improve |
| intended type | MISC |
| gameplay role | Official sampleware |
| reachability target | Existing item |
| implementation note | Cross-reference with items_4.md. |
...
- items_1.md:85: Official sampleware; Cross-reference with `items_4.md`.
- items_4.md:73: Official sample container; Keep as base for sample chain.
...
- src/data/item_packs/nii_sample_container.ts
...
- Add or improve only `nii_sample_container`.
...
- Whether `nii_sample_container` was added, improved, merged into an existing id or rejected.
