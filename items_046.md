# Items 046: moskvin_rifle

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `moskvin_rifle` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:96 (Liquidator weapons, ammo and combat roles)
...
| gameplay role | Slow accurate liquidator rifle |
| reachability target | 3rd-rank liquidator trade |
| implementation note | Must differ from nagant and harpoon_gun. |
...
- src/data/item_packs/moskvin_rifle.ts plus src/data/weapon_packs/moskvin_rifle.ts for weapons
...
- Add or improve only `moskvin_rifle`.
...
- Whether `moskvin_rifle` was added, improved, merged into an existing id or rejected.
