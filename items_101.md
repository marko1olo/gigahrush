# Items 101: labor_shift_card

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `labor_shift_card` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:93 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Worker access / factory gate |
| reachability target | Factory/office |
| implementation note | Links to production floors. |
...
- src/data/item_packs/labor_shift_card.ts
...
- Add or improve only `labor_shift_card`.
...
- Whether `labor_shift_card` was added, improved, merged into an existing id or rejected.
