# Items 044: party_might_launcher

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `party_might_launcher` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:94 (Liquidator weapons, ammo and combat roles)
...
| Field | Value |
| --- | --- |
| id | `party_might_launcher` |
| Russian name | Подствольник «Мощь партии» |
| mode | new |
| intended type | WEAPON |
| gameplay role | Rifle grenade / breach mod |
| reachability target | Rare liquidator stash |
| implementation note | Model as standalone launcher if no attachments. |
...
- src/data/item_packs/party_might_launcher.ts plus src/data/weapon_packs/party_might_launcher.ts for weapons
...
- Add or improve only `party_might_launcher`.
...
- Whether `party_might_launcher` was added, improved, merged into an existing id or rejected.
