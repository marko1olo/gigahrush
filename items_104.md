# Items 104: mail_intercept_slip

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `mail_intercept_slip` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:96 (Documents, permits, bureaucracy and access goods)
...
| mode | new |
| intended type | MISC |
| gameplay role | Theft vs legal delivery |
| reachability target | Office/black market |
| implementation note | Decision item. |
...
- src/data/item_packs/mail_intercept_slip.ts
...
- Add or improve only `mail_intercept_slip`.
...
- Whether `mail_intercept_slip` was added, improved, merged into an existing id or rejected.
